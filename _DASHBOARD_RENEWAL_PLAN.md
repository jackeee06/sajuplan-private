...
| 7차 (2026-05-19) | 상담사 운영 패널 Row 신설 (오늘 활성/이탈위험/미답변) + KPI 9개로 별점 추가 | 진행 중 |

---

## 7차 — 정보 풍부도 보강 (사장님 피드백: "정보가 부족하다")

**신규 endpoint**:
- `GET /admin/dashboard/counselor-panel` — 운영자 매일 챙길 핵심 명단 3종
- `GET /admin/dashboard/quality-kpi` — 평균 별점 + 낮은 별점 카운트

**Row 신설** (상담사 운영 패널, 3컬럼):
- 오늘 활성 상담사 TOP5 (오늘 상담 건수 많은 순)
- 이탈 위험 (7일 0건) — 활성 상담사 중 7일 활동 없는 사람
- 미답변 후기 5건 — 별점 + 본문 미리보기

**KPI 8 → 9개**:
- 추가: 평균 별점(30일) ★ X.XX + 낮은 별점 N건
- 톤: 4.5+ emerald / 4.0+ amber / 그 외 rose

**향후 2~3 단계 후보 (의견 정리)**:

### 2단계 — 의사결정 인사이트
- 이번주 vs 지난주 비교 카드 (매출/상담/가입 +X%/-X%)
- 채널별 매출 도넛 (070/060/채팅/충전 비중)
- 시간대별 트래픽 차트 (24시간 분포)
