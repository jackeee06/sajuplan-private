...
function display_banner($position, $skin='')
{
    ...
    if(file_exists($skin_path)) {
        // 접속기기
       // $sql_device = " and ( bn_device = 'both' or bn_device = 'pc' or bn_device='' ) ";
        if(G5_IS_MOBILE)
            //$sql_device = " and ( bn_device = 'both' or bn_device = 'mobile' or bn_device='' ) ";
...
/// 후기 갯수 가져오기
function get_item_use_cnt($it_id){
	if(!$it_id)return;
	$sql = "select count(*) as ct from g5_shop_item_use where it_id='".$it_id."'";
	$row = sql_fetch($sql);
	return $row["ct"];
...
// 문의 갯수 가져오기
function get_item_qa_cnt($it_id){
	if(!$it_id)return;
	$sql = "select count(*) as ct from g5_shop_item_qa where it_id='".$it_id."'";
	$row = sql_fetch($sql);
	return $row["ct"];
}