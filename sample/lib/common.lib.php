...
function send_mjson_auto_pay($murl, $data, $mode, $mb_1=""){
    ...
    global $CPID, $headerKey;
    $apiUrl = "http://passcall.co.kr:32837/";
...
function get_my_con_list($mb_id, $csr_id){
    ...
    $sql = "select * from platform_consulting where membid='".$minfo["mb_1"]."' and csrid='".$cinfo["mb_1"]."'  and reason='DISCONNECT' order by wr_datetime desc";
    $result = sql_query($sql);
...
function get_counselor_counter($csr_id){
    ...
    if($cinfo["mb_1"]){
        ...
        $sql ="select count(*) as ct from platform_consulting where csrid='".$cinfo["mb_1"]."' and wr_datetime >='".$tmonth."'  and reason='DISCONNECT'";
        //echo $sql;
        $row=sql_fetch($sql);
        return $row["ct"];
    }
...
function get_counselor_counter_all($csr_id){
    ...
    $cinfo = get_member($csr_id);
    if($cinfo["mb_1"]){
        $sql ="select count(*) as ct from platform_consulting where csrid='".$cinfo["mb_1"]."' and reason='DISCONNECT'";
        $row=sql_fetch($sql);
        return $row["ct"];
    }
}
...
function get_counselor_sum_time($csr_id){
    if(!$csr_id)return;
    $sql = "select sum(usetm) as tm from platform_consulting where mb_id='".$csr_id."'";
    $row = sql_fetch($sql);
    return $row["tm"];
}
...
function get_week_con_num(){
    ...
    $sql = " select count(*) as ct from platform_consulting  where wr_datetime >= '".$week."' and reason='DISCONNECT'";
...
function get_con_total_account_befre_mode($mb_id, $mode){
    ...
    // 지난달 060금액
    $sql = "select sum(amt) as p060 from platform_consulting where mb_id='".$mb_id."' and preflag='' and wr_datetime between '".$month_s."' and '".$month_d."'";
...
function set_con_account_v2($mb_id, $test_only=false, $month=''){
    ...
    try{
        ...
        // 기타 포인트: 상담(@platform_consulting)과 정산차감(@member) 제외한 모든 포인트
        $other = sql_fetch("
            SELECT 
            ...
            WHERE gp.mb_id='{$mb_id}'
              ...
              AND gp.po_rel_table NOT IN ('@member', '@platform_consulting')
        ");
...
function set_con_account_v3($mb_id, $month = ''){
    ...
    try {
        ...
        // platform_consulting 집계 (환불 건 제외: DISCONNECT + usetm<30 + amt<=mb_4)
...
function get_con_pay_sum($mb_1, $mode='', $mb_hp=''){
    ...
    $sql = "select sum(amt) as price from platform_consulting where (1) and ({$where_member}) {$sql_mode}";
...
function get_donot_my_c_history_cnt(){
    ...
    if($member["mb_level"]=="5"){
        $sql = "select no from platform_consulting where (1) and reason='DISCONNECT' and csrid='".$member["mb_1"]."'";
        //echo $sql;
...
function get_donot_my_review_cnt(){
    ...
    if($member["mb_level"]=="2"){
        $sql = "select no from platform_consulting where (1) and reason='DISCONNECT' and membid='".$member["mb_1"]."'";
        //echo $sql;
