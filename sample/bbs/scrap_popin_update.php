...
include_once(G5_PATH.'/head.sub.blank.php');
...
////////////////// 카운셀러 페이지에서 스크랩하면 해당 카운셀러 즐겨찾기 숫자 증가해준다 ////
if($bo_table=="counselor"){
	$crow = sql_fetch("select * from g5_write_counselor where wr_id='".$wr_id."'");
	if($crow["mb_id"]){
		$usql = "update g5_write_counselor set fat=fat+1 where wr_id='".$wr_id."'";
		sql_query($usql);
	}
}
