...
//20250801 eun 검색결과 정렬 작업 시작
        //$sql = " select * from {$tmp_write_table} where {$sql_search} order by wr_id desc limit {$from_record}, {$rows} ";
        $sql = " select w.*, m.state from {$tmp_write_table} w join g5_member m on w.mb_id = m.mb_id where {$sql_search} 
                 order by if (m.state in('IDLE', 'RDVC', 'CONN', 'ABSE','RDCH'), 0, 1), field (m.state,'IDLE', 'RDVC', 'CONN', 'ABSE','RDCH','')  limit {$from_record}, {$rows} ";
//20250801 eun 검색결과 정렬 작업 마감 //20251205
        // echo $sql;
        //echo "<br>";
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            $list[$idx][$i]['ca_name'] = $row["ca_name"];
            $list[$idx][$i]['wr_1'] = $row["wr_1"];
            $list[$idx][$i]['wr_2'] = $row["wr_2"];
            $list[$idx][$i]['wr_3'] = $row["wr_3"];
            $list[$idx][$i]['wr_4'] = $row["wr_4"];
...
// echo $search_skin_path;
//echo "<br>";

///home/hosting_users/dfsoft_thesaju/www/theme/basic/mobile/skin/search/basic
