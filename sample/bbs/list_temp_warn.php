...
if($state){
		$ss = "select mb_id from g5_member as m where state='".$state."' and mb_level='5'"; //20250715 정렬 및 페이징 테이블 alias 작업 시작
		$rst = sql_query($ss);
		if($rst){
			$mbid = array();
...
if ($sca || $stx || $stx === '0') {     //검색이면
	...
	//20250714 eun 상담사 리스트 > 카테고리 적용 상태값에 따라 정렬 작업 시작

    //오류나면 10시 30분 항목까지 revert 할 것!!
    // mb_id 필터용 SQL 조각
    $mb_id_in = "";
    if (count($mb_ids) > 0) {
        // COUNT 쿼리용 (alias 없이)
	...
	/*원래코드if(count($mbid)>0){
			$sql_search .=" and mb_id in('".implode("','",$mbid)."')";   //0715  홈 - 상담사 전체보기로 들어왔을 때. sca 없음
	}*/
	...
	if($bo_table=="counselor"){ /////////////////////////// 상담 게시판일때 상담중 , 상담준비 상담사만 가져오기.
        ...
        if(!$state){
			...
			$qs = "select m.mb_id from {$write_table} as gwc left join g5_member as m on gwc.mb_id = m.mb_id where m.mb_level='5' 
                and (m.state='IDLE' or m.state='CONN' or m.state='ABSE'){$imss} order by FIELD (m.state, 'IDLE', 'CONN', 'ABSE')";
			...
			if(count($mb_ids)>0){
				$sql_search .=" and m.mb_id in('".implode("','",$mb_ids)."')";
			}
		}
	}
	//////////////// 상담 게시판일때 , 상담중, 상담준비 상담사만 가져오기 끝 ...
...
if ($_SERVER['REMOTE_ADDR'] == "115.93.39.5") {
    echo "[DEBUG] mb_id: {$mbid}\n";
    echo "[DEBUG] mb_ids: {$mb_ids}\n";
    echo "[DEBUG] total_count: {$total_count}\n";
    echo "[DEBUG] total_page: {$total_page}\n";
    echo "[DEBUG] page_rows: {$page_rows}\n";
    echo "[DEBUG] from_record: {$from_record}\n";
    echo "[DEBUG] list_page_rows: {$list_page_rows}\n";
...
if($bo_table=="counselor"){
	...
	if($s_desc=="wr_datetime"){
		$sql_order = " order by FIELD (m.state, 'IDLE', 'CONN', 'ABSE'), wr_datetime desc";
        echo $sql_order;
	}elseif($s_desc=="aft"){
		$sql_order = " order by FIELD (m.state, 'IDLE', 'CONN', 'ABSE'), aft desc";
	}elseif($s_desc=="fat"){
		$sql_order = " order by FIELD (m.state, 'IDLE', 'CONN', 'ABSE'), fat desc";
	}elseif($s_desc=="amt"){
		$sql_order = " order by FIELD (m.state, 'IDLE', 'CONN', 'ABSE'), amt asc";
	}elseif($s_desc=="damt"){
		$sql_order = " order by FIELD (m.state, 'IDLE', 'CONN', 'ABSE'), amt desc";
	}
    else {
        $sql_order = " order by FIELD (m.state, 'IDLE', 'CONN', 'ABSE')";
    }
}
...
if ($is_search_bbs) {
    ...
    $sql = " select distinct gwc.wr_parent from {$write_table} as gwc left join g5_member as m on gwc.mb_id = m.mb_id where {$sql_search} {$sql_order} limit {$from_record}, $page_rows ";
    if($_SERVER['REMOTE_ADDR'] == "115.93.39.5") {
      //  echo '<pre style="background:#f5f5f5;padding:10px;">[SEARCH BRANCH SQL]' . "\n" . htmlspecialchars($sql) . '</pre>';
    }
} else {
   // 원래 쿼리 3 $sql = " select * from {$write_table} as gwc left join g5_member as m on gwc.mb_id = m.mb_id where gwc.wr_is_comment = 0 {$sql_search}";
    $sql = "select * from {$write_table} as gwc left join g5_member as m on gwc.mb_id = m.mb_id where gwc.wr_is_comment = 0 {$mb_id_in_list} {$sql_search}";
    if(!empty($notice_array))
        $sql .= " and wr_id not in (".implode(', ', $notice_array).") ";
    $sql .= " {$sql_order} limit {$from_record}, $page_rows ";
    if($_SERVER['REMOTE_ADDR'] == "115.93.39.5") {

     //   echo '<pre style="background:#eef5ff;padding:10px;">[LIST BRANCH SQL]' . "\n" . htmlspecialchars($sql) . '</pre>';
    }
}
