...
$mb_time       = isset($_POST['mb_time'])         ? trim($_POST['mb_time'])       : "";
$mb_homepage    = isset($_POST['mb_homepage'])      ? trim($_POST['mb_homepage'])    : "";
$mb_tel         = isset($_POST['mb_tel'])           ? trim($_POST['mb_tel'])         : "";
$mb_hp          = isset($_POST['mb_hp'])            ? trim($_POST['mb_hp'])          : "";
// 인증로직 추가
$a_num          = isset($_POST['a_num'])            ? trim($_POST['a_num'])          : "";
...
if ($w == '' || $w == 'u') {
    ...
    if ($w == '') {
        ...
        $clean_hp  = preg_replace('/[^0-9]/', '', $mb_hp);
        $clean_num = preg_replace('/[^0-9]/', '', $a_num);

        $sql_auth = " SELECT a_idx FROM sms_auth
                    WHERE REPLACE(a_hp, '-', '') = '{$clean_hp}'
                        AND a_num = '{$clean_num}'
                        AND a_regdate >= DATE_SUB(NOW(), INTERVAL 10 MINUTE)
                    ORDER BY a_idx DESC
                    LIMIT 1 ";
        $row_auth = sql_fetch($sql_auth);

        if (empty($row_auth['a_idx'])) {
            alert('휴대폰 인증정보가 유효하지 않습니다. 다시 인증해주세요.');
        }
    }
    ...
    if ($w=='') {
		...
		if ($msg = exist_mb_hp($mb_hp, $mb_id))     alert($msg);
	...
	if($rtn){
			...
			if($jresult1["req_result"]=="00"){ /// 등록성공
					if($jresult1["membid"]){ /// 회원번호 업데이트
						$membid = "";
						$membid = sprintf("%06d",$jresult1["membid"]);
						$usql = "update {$g5['member_table']} set mb_1='".$membid."' where mb_id='".$mb_id."'";
						@sql_query($usql);
					}
			}else{ /// 등록실패
				  

				  if($jresult1["resultmessage"]){
					
					$dsql ="delete from g5_member where mb_id='".$mb_id."'";
					@sql_query($dsql);
					$dsql1 ="delete from g5_point where mb_id='".$mb_id."'";
					@sql_query($dsql1);

					alert($jresult1["resultmessage"],'/');
					exit;
		...
		$msql = "select DISTINCT b.mb_hp, a.* from tbl_android_phone a left join g5_member b on(a.t_phone=replace(b.mb_hp,'-','')) where b.mb_id='".$mb_id."'"; ///회원에게 푸쉬 보냄
		$mresult = sql_query($msql);
		if($mresult){
			while($mres=sql_fetch_array($mresult)){
				sendPushNotification($mres["t_android_id"], $link, $title, $message);
			}
		}
        
		set_member_push_board($message, $link, "alim_set1", $mb_id);	
        // 
        // 푸쉬 발송 끝 //
		...
		$bizmsg->usr_id           = $mb_id;
		$bizmsg->usr_name           = $mb_name;
		$bizmsg->usr_nick           = $mb_nick;
		$rr = $bizmsg->send(); 
...
$mb = get_member($mb_id);
	 ...
	 if(!$mb["mb_1"]){
			//////////////// passnet 회원등록 ///////////////
			$data = '{"membnm":"'.$mb["mb_name"].'","telno":"'.str_replace("-","",$mb["mb_hp"]).'","amt":'.$mb["mb_point"].'}';
			$murl1 = "memb-mgr";
			$jresult1 = send_mjson($murl1, $data, 'POST');
			if($jresult1["req_result"]=="00"){ /// 등록성공
					if($jresult1["membid"]){ /// 회원번호 업데이트
...
if ($w == '') {
	...
	goto_url(G5_HTTP_BBS_URL.'/register_result.php'); //20250528 회원가입 완료 페이지 추가
	// ※※ 24.10.16 Be careful to modify or delete. ※※
	//setcookie("y_SignUp", "ok", time() + 3600, "/");
	//setcookie("y_SignUpID", "$member['mb_id']", time() + 3600, "/");
?>
<!-- NAVER SCRIPT START -->
