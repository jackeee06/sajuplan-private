...
//20250806 eun 상담메모 작성 분기 시작
$c_url = isset($_REQUEST['c_url']) ? $_REQUEST['c_url'] : '';
//20250806 eun 상담메모 작성 분기 마감
...
if ($w == '') {
    ...
    if ($member['mb_level'] < $board['bo_write_level']) {
        if ($member['mb_id']) {
            alert('회원전용 서비스입니다.'.$board['bo_write_level']);
        } else {
            //alert("회원전용 서비스입니다.\\n회원이시라면 로그인 후 이용해 보십시오.", G5_BBS_URL.'/login.php?'.$qstr.'&amp;url='.urlencode($_SERVER['SCRIPT_NAME'].'?bo_table='.$bo_table));
            alert("회원전용 서비스입니다.\\n로그인 후 이용해 보십시오.", G5_BBS_URL.'/login.php?'.$qstr.'&amp;url='.urlencode($_SERVER['SCRIPT_NAME'].'?bo_table='.$bo_table.$qstr));
        }
    }
    ...
    if($member['mb_id'] && $write['mb_id'] === $member['mb_id']) {
        ...
        if ($member['mb_id']) {
            ...
            alert('글을 수정할 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.', G5_BBS_URL.'/login.php?'.$qstr.'&amp;url='.urlencode($_SERVER['SCRIPT_NAME'].'?bo_table='.$bo_table.$qstr));
        }
    }
...
/*if(!is_mobile() || defined('G5_IS_MOBILE_DHTML_USE') && G5_IS_MOBILE_DHTML_USE)
    $is_dhtml_editor_use = true;*/
if(!is_mobile() || true)  // 모바일에서도 항상 true 만들기
    $is_dhtml_editor_use = true;
//20251117 eun 모바일에서 에디터 보이게 마감
...
// 20250820 eun  상담사 소개(wr_4) 에디터 수정 시작
$wr4_content = isset($write['wr_4']) ? $write['wr_4'] : '';
// 수정 화면에서 비회원/비관리자에게 노출 시 필터를 적용하려면 아래 주석 해제
// if ($is_dhtml_editor && $w == 'u' && (! $is_member || ! $is_admin || $write['mb_id'] !== $member['mb_id'])) {
//     $wr4_content = get_text(html_purifier($wr4_content), 0);
// }
$editor_html_wr4 = editor_html('wr_4', $wr4_content, $is_dhtml_editor);
$editor_js      .= get_editor_js('wr_4', $is_dhtml_editor);
$editor_js      .= chk_editor_js('wr_4', $is_dhtml_editor);
// 20250820 eun  상담사 소개(wr_4) 에디터 수정 마감
