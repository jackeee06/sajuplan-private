...
if ($w == '' || $w == 'r') {
    if (isset($_SESSION['ss_datetime']) && $_SERVER['REMOTE_ADDR'] != '115.93.39.5') {
...
if ($w == '' || $w == 'r') {
    ...
    if($tmb_id && $bo_table=="counselor"){
        $member['mb_id'] = $tmb_id;
    }

    if(trim($wr_10) == "" && $bo_table == 'review' ){
        alert('후기등록 시 내역을 통해 작성해주세요.');
        exit;
    }

    // 리뷰 중복 체크
    if($bo_table=="review" && $_POST['wr_10'] != ""){
        $rv_query = "
        select 
         count(wr_id) as cnt
        from
        ...
        and
         wr_10 = '{$_POST['wr_10']}'
        ";
        $gwr_row = sql_fetch($rv_query);
        if((int)$gwr_row['cnt'] > 0){
            alert('해당 상담건에는 이미 작성한 리뷰가 있습니다.');
        }
    }
    ...
    if($bo_table=="review" && $wr_1){
        $rsql = "update g5_write_counselor set aft=aft+1 where mb_id='".$wr_1."'";
        sql_query($rsql);
        $config_row2 = sql_fetch("select * from g5_config where (1)");
        // 사용포인트가 1000이상일때만 지금

        $sql = " select * from platform_consulting where no = '{$_POST['wr_10']}'";
        $tc_row = sql_fetch($sql); // 0

        if((int)$config_row2['cf_2'] < (int)$tc_row['amt']){
            if($config_row2['cf_1'] > 0){
                insert_point($member['mb_id'], $config_row2['cf_1'], "{$board['bo_subject']} 후기 작성", $bo_table, $wr_id, '쓰기');
            }
        }
...
//20250806 eun 상담내역 url 분기 시작
/*$c_url = isset($_POST['c_url']) ? $_POST['c_url'] : '';*/
// 1) 로그로 확인 (PHP 에러 로그)
error_log("DEBUG: c_url = '{$c_url}'");

// 2) 화면에 바로 찍어보기 (개발 중에만 사용)
//echo "<pre>DEBUG: c_url = '{$c_url}'</pre>";
if($_POST['c_url'] == 'chat') {
    $redirect_url = './chat_record.php';
}
else if($_POST['c_url'] == 'call'){
    $redirect_url = './counselor_history.php';
}
else {
    $redirect_url = run_replace('write_update_move_url', short_url_clean(G5_HTTP_BBS_URL . '/board.php?bo_table=' . $bo_table . '&amp;wr_id=' . $wr_id . "&mymbid=" . $mymbid . $qstr), $board, $wr_id, $w, $qstr, $file_upload_msg);
}
