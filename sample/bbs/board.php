...
if ((isset($wr_id) && $wr_id) || (isset($wr_seo_title) && $wr_seo_title)) {
    ...
    if (!$write['wr_id'] && !$bo_table =="counselor") {
        $msg = '글이 존재하지 않습니다.\\n\\n글이 삭제되었거나 이동된 경우입니다.';
    ...
    // 자신의 글이거나 관리자라면 통과
    if (($write['mb_id'] && $write['mb_id'] === $member['mb_id']) || $is_admin ||($bo_table=="qa" && $write['wr_1']==$member["mb_id"]) || ($bo_table=="review" && $write['wr_1']==$member["mb_id"])) {
        ;
    } else {
        // 비밀글이라면
        if (strstr($write['wr_option'], "secret"))
        {
            ...
            {
                $sql = " select mb_id, mb_1 from {$write_table}
                ...
                if ($row['mb_id'] === $member['mb_id'] || $row["mb_1"]==$member["mb_id"]){ /// 글작성자 본인이거나, 상담사 이거나
                    $is_owner = true;
                }
            }
