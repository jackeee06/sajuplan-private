        ...
        if (f1.auth_num.value==""){
            ...
            $.post("/bbs/ajax_check.php", $("#sForm").serialize(),function(data){
                if (data=="Y"){
                    ...
                    // 1. 레이어 모달 방식 대응 (부모창 함수 호출)
                    if (window.parent && typeof window.parent.close_auth_modal === "function") {
                        window.parent.jQuery("input[name='hp_auth']").val("Y");
                        window.parent.jQuery("input[name='a_num']").val(authNum);
                        window.parent.close_auth_modal();
                    }
                    // 2. 일반 팝업 방식 대응
...
function closeWindow() {
    // 1. 부모창에 직접 만든 닫기 함수가 있을 경우 실행
    if (window.parent && typeof window.parent.close_auth_modal === "function") {
        window.parent.close_auth_modal();
    } 
    // 2. 함수가 없다면 부모창의 레이어 ID를 직접 찾아 삭제 (가장 확실함)
    else if (window.parent && window.parent.jQuery("#dynamic_auth_layer").length) {
        window.parent.jQuery("#dynamic_auth_layer").remove();
    }
    // 3. 혹시나 팝업으로 떴을 때 대비
    else {
        window.close();
