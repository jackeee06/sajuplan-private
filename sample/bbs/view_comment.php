...
$where = "";
if($bo_table=="counselor" && $member["mb_id"]){
    $where .=" and mb_id='".$member["mb_id"]."'";
}

$sql = " select * from $write_table where wr_parent = '$wr_id' and wr_is_comment = 1 {$where} order by wr_comment, wr_comment_reply ";
...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if($bo_table=="counselor" && !$member["mb_id"]){
        $row["wr_option"] = $row['wr_option'].",secret";
    }
    ...
    if (!strstr($row['wr_option'], 'secret') ||
        ...
        if(!get_session($ss_name))
            $list[$i]['content'] = '<a href="'.G5_BBS_URL.'/password.php?w=sc&amp;bo_table='.$bo_table.'&amp;wr_id='.$list[$i]['wr_id'].$qstr.'" class="s_cmt">비밀글입니다.</a>';
        else {
            $list[$i]['content'] = conv_content($row['wr_content'], 0, 'wr_content');
            $list[$i]['content'] = search_font($stx, $list[$i]['content']);
        }
