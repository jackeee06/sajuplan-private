...
<article id="bo_v" style="width:<?php echo $width; ?>">
    ...
    <section id="bo_v_atc">
        ...
        <div id="bo_v_con">
    		...
    		<!--<p style="margin-bottom:6px;"><span class="point"><?php echo $view['name'] ?></span>님의 소원</p>-->
    ...
    <header>
        <h2 id="bo_v_title" style="display:none;">
            ...
            <span class="bo_v_cate point"><?php echo $view['ca_name']; // 분류 출력 끝 ?></span> 
            <?php } ?>
            <span class="bo_v_tit"><?php echo cut_str(get_text($view['wr_subject']), 70); // 글제목 출력 ?></span>
        </h2>
        
        <div id="bo_v_info" style="padding:0 0 20px;  margin-top:0px;">
	        <h2>페이지 정보</h2>
	        <!--<span class="sound_only">작성자 </span><?php echo $view['name'] ?><span class="ip"><?php if ($is_ip_view) { echo "&nbsp;($ip)"; } ?></span>
            ·
            -->
            작성자: <?php echo $view['name'] ?><br />
	        기도 작성일: <?php echo date("y-m-d", strtotime($view['wr_datetime'])) ?><!--<i class="fa fa-clock-o" aria-hidden="true"></i> --><?php //echo date("y-m-d H:i", strtotime($view['wr_datetime'])) ?><br />
            기도 기간: <?php echo cut_str(get_text($view['wr_subject']), 70); // 글제목 출력 ?>일
            
	    </div>
        
        <div id="bo_v_info" style="padding:20px 0; border-top:1px solid #eee; margin-top:20px;">
