...
<section id="bo_w">
    <form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off">
        ...
        <div class="form_01 write_div">
            ...
            <div class="write_div counselor_02">
                <ul class="subject">상담사 약력 <span style=" font-weight:400;">(최대 1000글자)</span></ul>
                <ul class="item">
                    <label for="wr_7" class="sound_only">상담사 약력</label>
                    <textarea name="wr_7" id="wr_7"  rows="5" maxlength="1000"><?php echo $write['wr_7'] ?></textarea>
                </ul>
            </div>

            <!--20250820 eun 상담사 소개 이미지 추가 작업 시작-->

           <!-- <div class="write_div counselor_02">
                <ul class="subject">상담사소개 <span style=" font-weight:400;">(최대 1000글자)</span></ul>
                <ul class="item">
                    <label for="wr_4" class="sound_only">상담사소개<strong>필수</strong></label>
                    <textarea name="wr_4" id="wr_4" rows="5" maxlength="1000"><?php /*echo $write['wr_4'] */?></textarea>
                </ul>
            </div>-->
            <div class="write_div counselor_02">
                <ul class="subject">상담사 소개</ul>
                <ul class="item">
                    <label for="wr_4" class="sound_only">상담사 소개</label>
                    <?php echo $editor_html_wr4; //  에디터로 교체(이미지/서식 지원) ?>
                </ul>
            </div>
   

            <!--20250721 eun 상담사 프로필 이미지 gif 등록 작업 시작-->

            <div class="write_div counselor_02">
                <ul class="subject">상담사 썸네일</ul>
                <ul class="item">
                    <?php
                    // 허용할 MIME 타입 정의
                    $imageTypes  = 'image/png,image/gif,image/jpeg';
                    $detailTypes = 'image/png,image/gif,image/jpeg,video/mp4';

                    for ($i = 0; $is_file && $i < $file_count; $i++) {
                        // i==0 은 썸네일, 그 외는 상세(detail)
                        $accept = ($i === 0) ? $imageTypes : $detailTypes;
                        ?>
                        <div class="bo_w_flie write_div">
                            <div class="file_wr write_div filebox">
                                ...
                                <label for="bf_file_<?php echo $i+1 ?>">
                                    <i class="fa fa-download lb_icon" aria-hidden="true"></i>
                                    <span class="sound_only">파일 #<?php echo $i+1 ?></span>
                                    <span class="btn_file">파일첨부</span>
                                </label>
                                <input
                                        type="file"
                                        name="bf_file[]"
                                        id="bf_file_<?php echo $i+1 ?>"
                                        class="frm_file uploadBtn"
    ...
    function fwrite_submit(f)
    {
        ...
        $.ajax({
            ...
            data: {
                ...
                "content": (f.wr_content ? f.wr_content.value : '') + "\n" + (f.wr_4 ? f.wr_4.value : '') //20250820 eun 상담사 소개 부분도 검사하게 추가
            },
            dataType: "json",
            async: false,
            cache: false,
