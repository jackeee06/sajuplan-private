...
<?php if ($is_comment_write) {
    ...
    <aside id="bo_vc_w">
        ...
        <input type="hidden" name="wr_secret" class="selec_chk" id="wr_secret" value="secret" checked="checked">
		 <?php if (!$is_guest) { ?>
				<?php if ($comment_min || $comment_max) { ?><strong id="char_cnt"><span id="char_count"></span>글자</strong><?php } ?>
				<textarea id="wr_content" name="wr_content" required title="댓글 내용"
				<?php if ($comment_min || $comment_max) { ?>onkeyup="check_byte('wr_content', 'char_count');"<?php } ?> placeholder="상담사 답변은 30자 이내로 제한되므로, 상담가능 시간 등의 간단한 문의만 남겨 주시길 바랍니다."  maxlength="32"  ><?php echo $c_wr_content; ?></textarea>
				<?php if ($comment_min || $comment_max) { ?><script> check_byte('wr_content', 'char_count'); </script><?php } ?>
        <?}else{?>
				<?php if ($comment_min || $comment_max) { ?><strong id="char_cnt"><span id="char_count"></span>글자</strong><?php } ?>
				<textarea id="wr_content" name="wr_content" required title="댓글 내용"
				<?php if ($comment_min || $comment_max) { ?>onkeyup="check_byte('wr_content', 'char_count');"<?php } ?> placeholder="상담사 답변은 30자 이내로 제한되므로, 상담가능 시간 등의 간단한 문의만 남겨 주시길 바랍니다."  maxlength="32"  onclick="pag_rdr('<?=$_SERVER["REQUEST_URI"]?>');"><?php echo $c_wr_content; ?></textarea>
				<?php if ($comment_min || $comment_max) { ?><script> check_byte('wr_content', 'char_count'); </script><?php } ?>
		<?}?>
        ...
        <div class="bo_vc_w_wr">
            <div class="bo_vc_w_info">
                <?php if ($is_guest) { ?>
				<script>
				function pag_rdr(url){
					alert('로그인하셔야합니다!');
					location.href='/bbs/login.php?url='+url;
				}
				</script>
                <label for="wr_name" class="sound_only">이름<strong> 필수</strong></label>
                <input type="text" name="wr_name" value="<?php echo get_cookie("ck_sns_name"); ?>" id="wr_name" required class="frm_input required" size="25" placeholder="이름" onclick="pag_rdr('<?=$_SERVER["REQUEST_URI"]?>');">
                <label for="wr_password" class="sound_only">비밀번호<strong> 필수</strong></label>
                <input type="password" name="wr_password" id="wr_password" required class="frm_input required" size="25"  placeholder="비밀번호" onclick="pag_rdr('<?=$_SERVER["REQUEST_URI"]?>');">
                <?php
                }
                ?>
                <?php if ($is_guest) { ?>
                ...
                <span class="bo_vc_secret chk_box">
                    <input name="wr_secret" type="checkbox" class="selec_chk" id="wr_secret" value="secret" checked="checked">
            ...
            <div class="btn_confirm">
                <button type="submit" id="btn_submit" class="btn_submit">상담문의 작성하기</button>
            </div>
...
<section id="bo_vc">
        ...
        <div class="review_sort" >
	    	<ul class="review_sort_item">
    	    	 <h2 class="bo_vc_tit">문의 내역 <span><?php echo $view['wr_comment']; ?></span>건</h2>
	        </ul>
    	    <ul class="review_sort_photo">
        		<input type="checkbox" id="ing_counsel">
	        	<label for="ing_counsel">나의 문의</label>
	        </ul>
    	</div>
    ...
    <article id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?> class="re_comment" <?php } ?>>
        <div class="comment_inner">
            <header>
                ...
                <span class="bo_vc_hdinfo"><time datetime="<?php echo date('Y-m-d', strtotime($list[$i]['datetime'])) ?>"><?php echo $list[$i]['datetime'] ?></time></span>
                ...
                <div class="bo_vl_opt">
                    <button type="button" class="btn_cm_opt btn_b03 btn" onclick="commentDisplay(this)"><i class="fa fa-ellipsis-v" aria-hidden="true"></i><span class="sound_only">댓글 옵션</span></button>
                    <ul class="bo_vc_act" style="display:none">
                        ...
                        <?php if ($list[$i]['is_del']) { ?><li><a href="<?php echo $list[$i]['del_link']; ?>" onclick="return comment_delete(this);">삭제</a></li><?php } ?>
                    </ul>
                </div>
                <?php } ?>
            </header>
...
// 원본스킨 - na1129 님 : https://sir.kr/g5_plugin/4972
...
<!-- wittazzurri 버튼옵션 수정 시작 : https://sir.kr/g5_tip/21028 -->
	<script>
