...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if (file_exists($bimg))
    {
        ...
        echo $banner.'<img src="'.G5_DATA_URL.'/banner/'.$row['bn_id'].'" width="'.$size[0].'" alt="배너"'.$bn_border.'>';
        if($banner)
            echo '</a>'.PHP_EOL;
        echo '</li>'.PHP_EOL;
...
<style>
#sbn_idx<?php echo $idx;?> {position:relative;margin:0 0 0px}
#sbn_idx<?php echo $idx;?> .sbn_btn {    z-index: 3;
    position: absolute;
    opacity:0;
    width: 40%;
    background: #fff;
    height: 100%;} /* 슬라이드 좌우 버튼 => 투명, 좌우 40% 선택 시 넘어가도록 커스텀 */
...
#sbn_idx<?php echo $idx;?> .sbn_btn span {display:block;width:100%;height:100%;background-color:red;background:url('<?php echo G5_SHOP_SKIN_URL;?>/img/sbn_btn.png')}
#sbn_idx<?php echo $idx;?> #sbn_btn_p<?php echo $idx;?> {left:0px}
#sbn_idx<?php echo $idx;?> .sbn_btn #sbn_btn_prev<?php echo $idx;?> span {background-position:0 0}
#sbn_idx<?php echo $idx;?> #sbn_btn_n<?php echo $idx;?> {right:0px}
#sbn_idx<?php echo $idx;?> .sbn_btn #sbn_btn_next<?php echo $idx;?> span {background-position:-70px 0}
