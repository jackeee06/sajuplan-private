...
$is_social = "N";
if(social_login_link_account($member["mb_id"],false,'get_data')){
    $is_social = "Y";
}else{
    $is_social = "N";
}
...
<input type="hidden" id="is_social" name="is_social" value="<?php echo $is_social ?>">
...
<input type="hidden" name="a_num"     value="">
<!-- 검증로직단계 추가 -->
...
<div class="top_nav" style="">
	...
	if($member["mb_level"]=="5"){
		...
		$purl = "";
		if($mrow["wr_id"]){
			$purl = "?w=u&bo_table=counselor&wr_id=".$mrow["wr_id"]."&tmb_id=".$member["mb_id"];
		}else{
			$purl = "?bo_table=counselor";
		}
		?>
			<a href="/bbs/write.php<?=$purl?>"><ul>상담사프로필111 <i class="xi-external-link"></i></ul></a>
		<?
		}
		?>
    ...
    <div id="info_01" >
        ...
        <ul>
            ...
            <li>
                <div style="display:flex;">
	                <input type="text" id="reg_mb_name" name="mb_name" value="<?php echo get_text($member['mb_name']) ?>" <?php echo $required ?> <?php echo $readonly; ?> class="frm_input full_input <?php echo $required ?> <?php echo $readonly ?>" size="10" placeholder="이름을 입력해주세요" style="width:calc(100% - 110px);">
                
    	            <p style="margin-top:0px; width:110px; text-align:right;">
                        <input type="radio" id="mb_10_1" required name="mb_10" value="남"<?php echo ($member['mb_10'] == "남") ? " checked" : "";?>> <label for="mb_10_1" style="width:auto;">남자</label>
            	        &nbsp;&nbsp;
                	    <input type="radio" id="mb_10_2" required name="mb_10" value="여"<?php echo ($member['mb_10'] == "여") ? " checked" : "";?>> <label for="mb_10_2" style="width:auto;">여자</label>
	                </p>
                </div>
                <?php
        ...
        <ul>
            ...
            <li>
                    ...
                    <input type="text" name="mb_nick" value="<?php echo isset($member['mb_nick'])?get_text($member['mb_nick']):''; ?>" id="reg_mb_nick" required class="frm_input required nospace  full_input" size="10" maxlength="20" placeholder="닉네임을 입력해주세요" <?if($member["mb_level"]=="5"){?>readonly onclick="alert('관리자에게 문의해주세요.');return false;" <?}?>>
        ...
        <ul>
            ...
            <?php if ($config['cf_use_hp'] || $config['cf_cert_hp']) {  ?>                
                ...
                <input type="text" name="mb_hp" value="<?php echo get_text($member['mb_hp']) ?>" id="reg_mb_hp" <?php echo ($config['cf_req_hp'])?"required":""; ?> class="frm_input  full_input <?php echo ($config['cf_req_hp'])?"required":""; ?>" maxlength="20" placeholder="휴대폰번호" <?if($w=='u'){ ?> readonly  onclick="alert('휴대폰번호 수정은 관리자에게 문의 바랍니다.');return false;" <?}?>>
		...
		<ul>
            ...
            <li>                
                <div style="display:flex;">
	                <input type="date" name="mb_birth" value="<?php echo isset($member['mb_birth'])?$member['mb_birth']:''; ?>" id="mb_birth" required class="frm_input full_input " size="70" maxlength="100" placeholder="2001-06-22" style=" width:calc(100% - 110px);">
					
                    <p style="margin-top:0px; width:110px; text-align:right;">
                        <input type="radio" id="mb_11_1" required name="mb_11" value="양력"<?php echo ($member['mb_11'] == "양력") ? " checked" : "";?> checked="checked"> <label for="mb_11_1" style="width:auto;">양력</label>
            	        &nbsp;&nbsp;
                	    <input type="radio" id="mb_11_2" required name="mb_11" value="음력"<?php echo ($member['mb_11'] == "음력") ? " checked" : "";?>> <label for="mb_11_2" style="width:auto;">음력</label>
	                </p>
                </div>
            </li>
        </ul>
        ...
        <!--
		<ul>
        	<label for="mb_time">태어난시간</label>
            <li>                
                <input type="text" name="mb_time" value="<?php echo isset($member['mb_time'])?$member['mb_time']:''; ?>" id="mb_time" required class="frm_input full_input " size="70" maxlength="100" placeholder="오후3시">
        ...
        <ul>
        	<label for="mb_time">주소</label>
            <li>
                <?php if ($config['cf_req_addr']) { ?><?php }  ?>
                    <!--<label for="reg_mb_zip">주소</label>-->
                    <p style="display:flex;">
                    <input type="text" name="mb_zip" value="<?php echo $member['mb_zip1'].$member['mb_zip2']; ?>" id="reg_mb_zip" <?php echo $config['cf_req_addr']?"required":""; ?> class="frm_input full_input frm_address <?php echo $config['cf_req_addr']?"required":""; ?>" size="10" maxlength="6"  placeholder="우편번호" style="width:calc(100% - 70px);" >
                    <button type="button" class="btn_frmline" onclick="win_zip('fregisterform', 'mb_zip', 'mb_addr1', 'mb_addr2', 'mb_addr3', 'mb_addr_jibeon');" style="width:70px;">주소검색</button><br>
                </p>
        ...
        <!-- 유입경로 -->
        <ul>
            <label for="mb_21">유입경로</label>
            <li>
                <textarea name="mb_21" id="mb_21" class="frm_input full_input" rows="3" style="width:100%; resize:vertical; color:#333;" placeholder="어떤 경로로 저희 서비스를 알게 되셨나요? (예: 지인추천, 검색, SNS 등)"><?php echo isset($member['mb_21']) ? get_text($member['mb_21']) : ''; ?></textarea>
                <style>
                    #mb_21::placeholder { color: #666; opacity: 1; }
                </style>
            </li>
        </ul>
...
<?php if($member['mb_level']>4){ //권한2보다 높은 권한일 경우 
	...
	$member["mb_8"]?$mb_8s = explode("|",$member["mb_8"]):$mb_8s=array();
...
<div id="register_form"  class="form_01">     
    <div>
		...
		<ul>
            ...
            <li>
                <input type="text" name="mb_8_1" value="<?php echo $mb_8s[0]?>" id="mb_8_1" class="frm_input full_input" maxlength="20" placeholder=" OO은행">
		...
		<ul>
            ...
            <li>
                <input type="text" name="mb_8_2" value="<?php echo $mb_8s[1] ?>" id="mb_8_2" class="frm_input full_input" maxlength="20" placeholder="김OO">
		...
		<ul>
            ...
            <li>
                <input type="text" name="mb_8_3" value="<?php echo $mb_8s[2] ?>" id="mb_8_3" class="frm_input full_input" maxlength="20" placeholder="00000000000">
...
<div id="register_form"  class="form_01">   
    ...
    <div>
        <ul>
            ...
            //     $mb_id = social_login_link_account($member["mb_id"],false,'get_data');
            //     var_dump($mb_id);
            // }
        ...
        <?php if($_SESSION['ss_social_provider']){ ?>
        <p class="social_guide">
			ⓘ 연동된 네이버/카카오톡 버튼을 선택, 로그아웃하시면 계정 연동이 끊깁니다.
...
<div id="register_form"  class="form_01 last">       
    ...
    <div>
        ...
        <?php if($w != '') { ?>
            ...
            <?php if($_SESSION['social_member_provider_manage']){ ?>
            ...
            <div class="btn_leave_wrap">
                <a href="javascript:void(0)" class="btn_leave" onclick="member_leave();">회원탈퇴</a>
            </div>
        <?php } // tto?>      
    </div>
...
<!-- <script>
        $("#win_hp_auth1").click(async function() {

            

            const cfm = await webConfirm('회원탈퇴를 하시겠습니까? 회원 정보가 모두 삭제됩니다.');
            var pw= $("#reg_mb_password").val();
            var pw1= $("#reg_mb_password_re").val();
            
            if(!pw || !pw1){
                webAlert('비밀번호를 입력하세요');
                return false;
            }

            if(cfm){
                $("#w").val("d");
                $("#fregisterform").attr("action",'/bbs/member_leave.php');
	...
	async function member_leave(){

        const cfm = await webConfirm('회원탈퇴를 하시겠습니까? 회원 정보가 모두 삭제됩니다.');
        var pw= $("#reg_mb_password").val();
        var pw1= $("#reg_mb_password_re").val();
        var is_social = $("#is_social").val();
		...
		// var cfm = confirm('회원탈퇴를 하시겠습니까? 회원 정보가 모두 삭제됩니다.');
		// var pw= $("#reg_mb_password").val();
		// var pw1= $("#reg_mb_password_re").val();
    ...
    $(function() {
        ...
        $("#win_hp_auth").click(function() {
            if($("#reg_mb_hp").val() == ""){
                webAlert("휴대폰번호를 입력해주세요.");
                return false;
            }
            opt = 'scrollbars=0,width=400,height=320,top=10,left=20';
            popup_window2(g5_bbs_url+"/sms.php?mb_hp="+$("#reg_mb_hp").val(), "sms_auth", opt);
        });
        <?php } ?>
        
        function popup_window2(url, winname, opt) {
            if ($("#dynamic_auth_layer").length) $("#dynamic_auth_layer").remove();

            var layerHtml = `
                <div id="dynamic_auth_layer" style="position:fixed; top:0; left:0; width:100%; height:100%; background:#fff; z-index:999999; display:flex; flex-direction:column;">
                    <div style="padding:15px; background:#242d3d; color:#fff; display:flex; justify-content:space-between; align-items:center; flex-shrink:0;">
                        ...
                        <span onclick="close_auth_modal()" style="cursor:pointer; font-size:28px; line-height:1; padding:0 10px;">&times;</span>
                    </div>
    ...
    function close_auth_modal() {
            if (jQuery("#dynamic_auth_layer").length) {
                jQuery("#dynamic_auth_layer").remove();
            }
     }
    ...
    function fregisterform_submit(f)
    {
		...
		if($("#w").val()=="d"){ // 삭제일때는 바로 보낸다.
			return true;
		}

		<?php if ($w == "") { ?>
		if (!f.agree.checked) {
		    webAlert("회원가입약관의 내용에 동의하셔야 회원가입 하실 수 있습니다.");
		    f.agree.focus();
		    return false;
		}

		if (!f.agree2.checked) {
		    webAlert("개인정보처리방침안내의 내용에 동의하셔야 회원가입 하실 수 있습니다.");
		    f.agree2.focus();
		    return false;
		}
        ...
        // 본인확인 체크
        if(f.cert_no.value=="") {
            webAlert("회원가입을 위해서는 본인확인을 해주셔야 합니다.");
            return false;
        }
        <?php } ?>
...
//            }
        ...
        if(f.hp_auth.value != "Y"){
            webAlert("회원가입을 위해서 휴대폰 인증을 해주세요.");
            return false;
