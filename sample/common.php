...
ini_set("session.gc_divisor", 1000); // session.gc_divisor는 session.gc_probability와 결합하여 각 세션 초기화 시에 gc(쓰레기 수거) 프로세스를 시작할 확률을 정의합니다. 확률은 gc_probability/gc_divisor를 사용하여 계산합니다. 즉, 1/100은 각 요청시에 GC 프로세스를 시작할 확률이 1%입니다. session.gc_divisor의 기본값은 100입니다.
...
//20250715 eun 상담사 리스트 필터링 url 유지 작업 시작
if($bo_table=="counselor"){
    if (isset($_REQUEST['s_wr_5']))  {
        $s_wr_5 = clean_xss_tags(trim($_REQUEST['s_wr_5']));
        if ($s_wr_5) {
            $s_wr_5 = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", $s_wr_5);
            //$qstr .= '&amp;csrid=' . urlencode($s_wr_5);
            $qstr .= '&amp;s_wr_5=' . urlencode($s_wr_5);
        }
    } else {
        $s_wr_5 = '';
    }

    if (isset($_REQUEST['s_wr_6']))  {
        $s_wr_6 = clean_xss_tags(trim($_REQUEST['s_wr_6']));
        if ($s_wr_6) {
            $s_wr_6 = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", $s_wr_6);
            //$qstr .= '&amp;csrid=' . urlencode($s_wr_6);
            $qstr .= '&amp;s_wr_6=' . urlencode($s_wr_6);
        }
    } else {
        $s_wr_6 = '';
    }

    if (isset($_REQUEST['s_mb_10']))  {
        $s_mb_10 = clean_xss_tags(trim($_REQUEST['s_mb_10']));
        if ($s_mb_10) {
            $s_mb_10 = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", $s_mb_10);
            #$qstr .= '&amp;csrid=' . urlencode($s_mb_10);
            $qstr .= '&amp;s_mb_10=' . urlencode($s_mb_10);
        }
    } else {
        $s_mb_10 = '';
    }
    ...
    if (isset($_REQUEST['state']))  {
        $state = clean_xss_tags(trim($_REQUEST['state']));
        if ($state) {
            $state = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", $state);
            $qstr .= '&amp;state=' . urlencode($state);
        }
    } else {
        $state .= '';
    }

    if (isset($_REQUEST['s_desc']))  {
        $s_desc = clean_xss_tags(trim($_REQUEST['s_desc']));
        if ($s_desc) {
            $s_desc = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", $s_desc);
            $qstr .= '&amp;s_desc=' . urlencode($s_desc);
        }
    } else {
        $s_desc = '';
    }
...
//20250715 eun 상담사 리스트 필터링 url 유지 작업 마감

if($bo_table=="review" || $bo_table=="qa"){
	if (isset($_REQUEST['csrid']))  {
		$csrid = clean_xss_tags(trim($_REQUEST['csrid']));
		if ($csrid) {
			$csrid = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", $csrid);
			$qstr .= '&amp;csrid=' . urlencode($csrid);
		}
	} else {
		$csrid = '';
	}

	if (isset($_REQUEST['csr_id']))  {
		$csr_id = clean_xss_tags(trim($_REQUEST['csr_id']));
		if ($csr_id) {
			$csr_id = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", $csr_id);
			$qstr .= '&amp;csr_id=' . urlencode($csr_id);
		}
	} else {
		$csr_id = '';
	}
...
if (isset($_REQUEST['mymbid']))  {
    $mymbid = clean_xss_tags(trim($_REQUEST['mymbid']));
    if ($mymbid) {
        $mymbid = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*]/", "", $mymbid);
        $qstr .= '&amp;mymbid=' . urlencode($mymbid);
    }
} else {
    $mymbid = '';
}
...
if (isset($_REQUEST['photo_view']))  {
    $photo_view = trim($_REQUEST['photo_view']);
    $photo_view = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*\s]/", "", $photo_view);
    if ($photo_view)

        $qstr .= '&amp;sst=' . urlencode($photo_view); 
} else {
    $photo_view = '';
}


if (isset($_REQUEST['re_counsel']))  {
    $re_counsel = trim($_REQUEST['re_counsel']);
    $re_counsel = preg_replace("/[\<\>\'\"\\\'\\\"\%\=\(\)\/\^\*\s]/", "", $re_counsel);
    if ($re_counsel)

        $qstr .= '&amp;sst=' . urlencode($re_counsel); 
} else {
    $re_counsel = '';
}
...
if($member){
	if($member["mb_level"]=="2" && !$member["mb_1"]){
		alert('회원 상담번호가 없습니다. 회원정보 수정을 눌러주세요~', '/bbs/register_form.php?w=u');
	}
}
////// 월 정산 //////////////
// 로그인했을때 정산처리를 하는부분이 있음
month_pay_execute();
