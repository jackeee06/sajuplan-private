...
function set_cookie($cookie_name, $value, $expire)
{
    ...
    // [테스트] 115.93.39.5: 로그아웃시 localStorage 클리어 플래그
    if(($cookie_name == 'ck_mb_id' || $cookie_name == 'ck_auto')){
        if(!$value || $expire <= 0) {
            if(isset($_SESSION)) $_SESSION['_g5_ck_cleared'] = true;
        }
    }
    // $_SERVER['REMOTE_ADDR'] == '115.93.39.5' && 
}
...
//echo "2";
    ...
    if($minfo["mb_level"]=="5"){ /// 회원 정보가 있으면 엠넷 포인트와 동기화 시켜준다. // 상담사 포인트
...
{
    global $g5, $config;
    // [2026-03-18] 포인트 소멸 정책 제거 — 소멸 로직 전체 주석처리
    /*
    $mb_check = get_member($mb_id, 'mb_level');
    $is_consultant = ($mb_check['mb_level'] == '5');
    if($config['cf_point_term'] > 0 && !$is_consultant) {
        ...
        if($expire_point > 0) {
            ...
            sql_query($sql);
            $minfo = get_member($mb_id);
            if($minfo['mb_level'] == '2' && $minfo['mb_1']){
                $data = '{"membnm":"'.$minfo['mb_name'].'","amt":'.$po_mb_point.'}';
                send_mjson("memb-mgr", $data, 'PUT', $minfo['mb_1']);
            }
        }
        $sql = " update {$g5['point_table']}
                    set po_expired = '1'
    ...
    */
    // 포인트합
    // 상담사(mb_level=5)는 2026-03-01 이후 포인트만 합산 (이전 내역 제외)
    $mb_check = get_member($mb_id, 'mb_level');
...
function member_delete($mb_id)
{
    ...
    $sql = " update {$g5['member_table']} set mb_password = '', mb_level = 1, mb_email = '', mb_homepage = '', mb_tel = '', mb_hp = '', mb_zip1 = '', mb_zip2 = '', mb_addr1 = '', mb_addr2 = '', mb_addr3 = '', mb_point = 0, mb_profile = '', mb_birth = '', mb_sex = '',mb_3 = '', mb_signature = '', mb_memo = '".date('Ymd', G5_SERVER_TIME)." 삭제함\n".sql_real_escape_string($mb['mb_memo'])."', mb_certify = '', mb_adult = 0, mb_dupinfo = '' where mb_id = '{$mb_id}' ";
...
///*
function get_csrid($csrid){
    if(!$csrid)return;
    $sql = "select * from g5_member where mb_1='".$csrid."'";
    $row = sql_fetch($sql);

    return $row;
}

function get_mbid($no){
    if(!$no)return;

    $no1 = (int)$no;
    $sql = "select * from g5_member where mb_1='".$no1."'";
    //echo $sql;
    $row = sql_fetch($sql);

    if(!$row["mb_id"]){
        $sql = "select * from g5_member where mb_1='".$no."'";
        //echo $sql;
        $row = sql_fetch($sql);
    }
...
function send_mjson($murl, $data, $mode, $mb_1=""){
...
function send_mjson_auto_pay($murl, $data, $mode, $mb_1=""){

    global $CPID, $headerKey;
    $apiUrl = "https://passcall.co.kr:32837/";
...
function jun_decode($response){
    $jdata = array();
    $pattern = '/\{(.*?)\}/';
    preg_match($pattern, $response, $data);
    if($data[1]){
        $data_arr = explode(",",str_replace("\"","",$data[1]));

        if(count($data_arr)>0){
            for($i=0;$i<count($data_arr);$i++){
                if($data_arr[$i]){
                    $tarr = explode(":",$data_arr[$i]);
                    $jdata[$tarr[0]] =$tarr[1];
                }
            }
        }
...
function send_mjson_cancle($murl, $data, $mode, $mb_1=""){

    global $CPID, $headerKey;
    $apiUrl = "https://passcall.co.kr:32837/";

    if($mode=="POST"){
        $url = $apiUrl.$murl."/gnrc_cancel_pay";
    }
...
//	exit;
    // 결과
    $retjson = json_decode(stripslashes($response),true);
    if($retjson==""){
        $retjson = jun_decode($response);
    }
...
function send_ch_json($murl, $data, $mode){

    global $CPID, $headerKey;

    $apiUrl = "http://passcall.co.kr:20102/";
...
/// 내가 해당 상담사 상담내역이 있는지 확인하고 리스트 뽑아줌
function get_my_con_list($mb_id, $csr_id){
    if(!$mb_id)return;
    if(!$csr_id)return;

    $minfo = get_member($mb_id);
    $cinfo = get_member($csr_id);

    $sql = "select * from thesaju_consulting where membid='".$minfo["mb_1"]."' and csrid='".$cinfo["mb_1"]."'  and reason='DISCONNECT' order by wr_datetime desc";
    $result = sql_query($sql);
...
//////////// 상담사 정보 가져오기 ///
function get_con_detail($no){
    if(!$no)return;

    $sql = "select * from g5_write_c_history where wr_10='".$no."'";
    //echo $sql;
    //echo "<br>";
    $crow = sql_fetch($sql);
...
//////////// 상담사 이미지 가져오기 ///
function get_con_img($mb_id, $w, $h){
    if(!$mb_id)return;
    $img = "";

    $sql = "select * from g5_write_counselor where mb_id='".$mb_id."'";
    $crow = sql_fetch($sql);

    $thumb1 = get_list_thumbnail('counselor', $crow['wr_id'], $w, $h, false, true);
    $img = "";
    if($thumb1['src']) {
        $img = $thumb1["src"];
    } else {
...
//// 상담후기 있는지 확인하기 ///
function get_is_review($no){
    if(!$no)return;
    $sql = "select wr_id from g5_write_review where wr_10='".$no."'";
    $row = sql_fetch($sql);
    return 	$row["wr_id"];
}
...
/// 실시간 내 페이지 보는사람 카운트 //
function get_mypage_view_count(){
    $url = $_SERVER["REQUEST_URI"];
    if($url){
        $sql = "select count(*) as ct from g5_login where lo_url='".$url."'";
        $row=sql_fetch($sql);

        $ct = (int)$row["ct"]+1;


        $sql1 = "select * from saju_config";
        $row1 = sql_fetch($sql1);

        if($row1["cf_1"]){
            $cnt = $cnt+(int)$row1["cf_1"];
        }
...
/// 최근 3개월 상담카운트 //
function get_counselor_counter($csr_id){
    if(!$csr_id)return;


    $cinfo = get_member($csr_id);
    if($cinfo["mb_1"]){
        $tmonth = date("Y-m-d H:i:s",strtotime("-3 month"));
        $sql ="select count(*) as ct from thesaju_consulting where csrid='".$cinfo["mb_1"]."' and wr_datetime >='".$tmonth."'  and reason='DISCONNECT'";
        //echo $sql;
        $row=sql_fetch($sql);
        return $row["ct"];
    }
...
//	if(!$csr_id)return;
//	$tmonth = date("Y-m-d H:i:s",strtotime("-3 month"));
//	$sql = "select count(*) as ct from g5_write_review where wr_1='".$csr_id."' and wr_datetime >='".$tmonth."'";
//	$row = sql_fetch($sql);
//
//	return $row["ct"];
//}
...
/// 모든 상담카운트 //
function get_counselor_counter_all($csr_id){
    if(!$csr_id)return;

    $cinfo = get_member($csr_id);
    if($cinfo["mb_1"]){
        $sql ="select count(*) as ct from thesaju_consulting where csrid='".$cinfo["mb_1"]."' and reason='DISCONNECT'";
        $row=sql_fetch($sql);
        return $row["ct"];
    }
}
...
//// 상담사 후기 갯수 카운팅 //
function get_counselor_afcnt($csr_id){
    if(!$csr_id)return;
    $cinfo = get_member($csr_id);
    $sql = "select count(*) as ct from g5_write_review where wr_1='".$csr_id."'";
    $row = sql_fetch($sql);
    return $row["ct"];
}
...
//// 상담사 문의 갯수 카운팅 //
function get_counselor_qa($csr_id){
    if(!$csr_id)return;
    $cinfo = get_member($csr_id);
    $sql = "select count(*) as ct from g5_write_review where wr_1='".$csr_id."'";
    $row = sql_fetch($sql);
...
//// 상담사 문의 갯수 카운팅 //
function get_counselor_qa_new($csr_id){
    if(!$csr_id)return;
    $cinfo = get_member($csr_id);
    $sql = "select count(*) as ct from g5_write_qa where wr_1='".$csr_id."'";
    $row = sql_fetch($sql);
    return $row["ct"];
}

//// [02/19] 추가 수정 속도가 느리므로 idx로 검색해야함
function get_dangol_cnt($wr_id){
    // if(!$wr_id)return;
    $sql = "select count(ms_id) as ct from g5_scrap where wr_id = '".$wr_id."'";
    $row = sql_fetch($sql);
    return $row['ct'];
}
...
/// 상담사 누적 상담시간 //
function get_counselor_sum_time($csr_id){
    if(!$csr_id)return;
    $sql = "select sum(usetm) as tm from thesaju_consulting where mb_id='".$csr_id."'";
    $row = sql_fetch($sql);
    return $row["tm"];
}

//// 상담사 단골수 가져오기 //
function get_counselor_scrap_count($wr_id){
    if(!$wr_id)return;
    $sql = "select count(*) as tm from g5_scrap where wr_id='".$wr_id."'";
    //echo $sql;
    $row = sql_fetch($sql);
    return $row["tm"];
}
...
/// 스크랩된 글인지 확인한다
function is_scrap_wr_id($mb_id, $bo_table, $wr_id){
    global $g5;
    $flag = false;
...
/// 현재 접속중인 상담사 숫자 //
function get_now_conn_con(){

    global $g5, $config;

    $sql = " select * from g5_member  where mb_level='5' and (state = 'IDLE' or state = 'ABSE' or state = 'RDVC' or state = 'RDCH') "; //20251205 RDCH 추가
    $result = sql_query($sql);
...
/// 최근일주일 상담 숫자 //
function get_week_con_num(){

    global $g5, $config;

    //일주일전
    $week = date("Y-m-d H:i:s",strtotime("-6 days"));

    $sql = " select count(*) as ct from thesaju_consulting  where wr_datetime >= '".$week."' and reason='DISCONNECT'";
    ...
    if($row1["cf_con_num"]){
        $cnt = $cnt+(int)$row1["cf_con_num"];
    }
    return $cnt;
}


function gola_value($vwr5){
    if(!$vwr5)return;
    $wr_5 = explode("|",$vwr5);
    $wr_5s = array();
    if(count($wr_5)>0){
        foreach($wr_5 as $key=>$value){
            if($value){
...
function get_cate_id($mb_id){
    global $g5;
    if(!$mb_id)return;

    $sql = "select ca_id from g5_shop_category where ca_mb_id='".$mb_id."'";
    $row = sql_fetch($sql);
...
///////////////// 상담사 전달 정산금액 가져오기 ///
function get_con_total_account_befre($mb_id){
    if(!$mb_id)return;

    $month = date("Y-m",strtotime("-1 month"));
    $sql = "select * from g5_point_end where mb_id='".$mb_id."' and month='".$month."'";
    $row = sql_fetch($sql);
    return $row["price"];
...
///////////////// 상담사 전달 정산금액 가져오기 060,070 ///
function get_con_total_account_befre_mode($mb_id, $mode){
    if(!$mb_id)return;


    $month = date("Y-m",strtotime("-1 month"));

    $month_s = date("Y-m",strtotime("-1 month"))."-01 00:00:00";
    $month_s1 = date("Y-m",strtotime("-1 month"))."-01";
    $last_day = date('t', strtotime($month_s1));
    $month_d = date("Y-m",strtotime("-1 month"))."-".$last_day;

    // 지난달 060금액
    $sql = "select sum(amt) as p060 from thesaju_consulting where mb_id='".$mb_id."' and preflag='' and wr_datetime between '".$month_s."' and '".$month_d."'";

    //echo $sql;
    //echo "<br>";

    $row = sql_fetch($sql);
    $sum_060 = (int)$row["p060"];


    $month = date("Y-m",strtotime("-1 month"));
    $sql = "select sum(po_point) as tprice from g5_point where po_content != '[수동] 11월 정산 중복' and po_content != '[수동]정산 시스템 오류로인한 미지급건' and mb_id='".$mb_id."' and po_datetime between '".$month_s."' and '".$month_d."'";
    //echo $sql;
    //echo "<br>";
    $row = sql_fetch($sql);
    $sum_all = (int)$row["tprice"];
    $sum_070 = $sum_all - $sum_060;
...
/////////////////// 상담사 이번달 보유금액 가져오기 ///////////
function get_con_total_account($mb_id , $md=""){
    if(!$mb_id)return;
    ...
    if(!$md){
        ...
        $sql = "select sum(po_point) as tprice from g5_point where po_content != '[수동] 11월 정산 중복' and po_content != '[수동]정산 시스템 오류로인한 미지급건' and mb_id='".$mb_id."' and po_datetime >='".$nowday."'";
        //echo $sql;
    }
    $row = sql_fetch($sql);

    $tprice = 0;
    $tprice = (int)$row["tprice"];
    if($tprice <0){
        $tprice = 0;
    }
...
function set_con_account_v2($mb_id, $test_only=false, $month=''){
    ...
    if(!$mb_id) return;
    /* =========================================================
       🔐 상담사(mb_id) 단위 DB LOCK
       같은 mb_id는 동시에 1개 프로세스만 실행
       
    ========================================================= */
    $lock_name = "SETTLE_".$mb_id;
    $lock = sql_fetch("SELECT GET_LOCK('{$lock_name}', 5) AS l");

    if(!$lock['l']){
        return; // 이미 다른 프로세스가 처리중
    }
    ...
    try{
        ...
        ========================================================= */
        $minfo    = get_member($mb_id);
        $bmonth   = $month ? $month : date("Y-m",strtotime("-1 month")); // 정산 대상 월
        $startday = $bmonth."-01 00:00:00";
        $endday   = date("Y-m-d", strtotime($bmonth."-01 +1 month"))." 00:00:00";
        $lastday  = date('t', strtotime($bmonth."-01"));
        $point_insert_day = $bmonth."-".$lastday." 23:59:59";

        // 정산 기준일: 이 날짜 이전 포인트는 0원 처리 (2026-03-01 신규 정산 시작)
        $point_base_date = '2026-03-01 00:00:00';


        if(!$member["mb_id"]) $member["mb_id"] = "system";

        // 디버그 로그 (확인 후 삭제)
        error_log("[set_con_account_v2] mb_id={$mb_id} bmonth={$bmonth} startday={$startday} endday={$endday}");
        ...
        $row = sql_fetch("
            ...
            WHERE mb_id='{$mb_id}'
              AND month='{$bmonth}'
        ");
        $already_exists = ((int)$row['ct'] > 0);
       


        /* =========================================================
           3. 전달 포인트 합계 계산 (포인트 차감용)
        ========================================================= */
        $row = sql_fetch("
        ...
        /* =========================================================
           3-1. 로열티 기반 정산금액 계산 (v3 로직)
        ========================================================= */
        $royalty_free = (int)$minfo['mb_19'];
        $royalty_pro  = (int)$minfo['mb_20'];

        // 환불 건 제외: DISCONNECT + usetm<30 + amt<=mb_4
        $mb_4 = (int)$minfo['mb_4'];
        $con = sql_fetch("
            SELECT 
                IFNULL(SUM(tc.amt_free), 0) as amt_free,
                IFNULL(SUM(tc.amt_pro), 0) as amt_pro
            FROM thesaju_consulting tc
            WHERE tc.mb_id='{$mb_id}'
              AND tc.reason IN ('DISCONNECT','END_CHAT')
              AND tc.wr_datetime >= '{$startday}' 
              AND tc.wr_datetime < '{$endday}'
              AND NOT (tc.reason = 'DISCONNECT' AND tc.usetm < 30 AND tc.amt <= '{$mb_4}')
              AND EXISTS (
                  SELECT 1 FROM g5_point gp
                  WHERE gp.c_no = tc.no
                    AND gp.mb_id = tc.mb_id
              )
        ");

        $amt_free = (int)$con['amt_free'];
        $amt_pro  = (int)$con['amt_pro'];

        // 기타 포인트: 상담(@thesaju_consulting)과 정산차감(@member) 제외한 모든 포인트
        $other = sql_fetch("
            SELECT 
                IFNULL(SUM(CASE WHEN gp.po_point > 0 THEN gp.po_point ELSE 0 END), 0) as amt_plus,
                IFNULL(SUM(CASE WHEN gp.po_point < 0 THEN gp.po_point ELSE 0 END), 0) as amt_minus
            FROM g5_point gp
            WHERE gp.mb_id='{$mb_id}'
              AND gp.po_datetime >= '{$startday}'
              AND gp.po_datetime < '{$endday}'
              AND gp.po_rel_table NOT IN ('@member', '@thesaju_consulting')
        ");
        $amt_other_plus  = (int)$other['amt_plus'];
        $amt_other_minus = (int)$other['amt_minus'];

        $price_free      = (int)floor($amt_free * $royalty_free / 100);
        $price_paid      = (int)floor($amt_pro * $royalty_pro / 100);
        $price_other     = (int)floor($amt_other_plus * $royalty_pro / 100) + $amt_other_minus;
        $price_tot       = $price_free + $price_paid + $price_other;
        $supply_price    = (int)floor($price_tot / 1.1);
        $vat_amount      = $price_tot - $supply_price;
        $withholding_tax = (int)floor($supply_price * 0.033);
        $reply_fee       = ($price_tot >= 50000) ? 20000 : 0;
        $p_price         = $supply_price - $withholding_tax - $reply_fee;

        // 디버그 로그 (확인 후 삭제)
        error_log("[set_con_account_v2] mb_id={$mb_id} amt_free={$amt_free} amt_pro={$amt_pro} other_plus={$amt_other_plus} other_minus={$amt_other_minus} price_free={$price_free} price_paid={$price_paid} price_tot={$price_tot} p_price={$p_price}");

        /* =========================================================
           4. 정산 결과 테이블 저장 (있으면 UPDATE, 없으면 INSERT)
        ========================================================= */
        if($already_exists){
            sql_query("
                UPDATE g5_point_end SET
                    price = {$p_price},
                    ...
                    amb_id = '{$member["mb_id"]}',
                    up_datetime = NOW()
                WHERE mb_id = '{$mb_id}' AND month = '{$bmonth}'
            ");
        } else {
            sql_query("
                INSERT INTO g5_point_end
                (mb_id, month, price, price_free, price_paid, price_other, price_tot, vat_amount, withholding_tax, reply_fee, amb_id, wr_datetime)
                VALUES
                ('{$mb_id}', '{$bmonth}', {$p_price}, {$price_free}, {$price_paid}, {$price_other}, {$price_tot}, {$vat_amount}, {$withholding_tax}, {$reply_fee}, '{$member["mb_id"]}', NOW())
            ");
        }
        

        /* =========================================================
           5. 포인트 차감 (신규일 때만, 중복 차감 방지)
        ========================================================= */
        if(!$test_only && $price > 0){
            ...
            $rel_action = ($p_price > 0)
                ? $bmonth."월 정기정산"
                : $bmonth."월 정기정산 10,000원 이하 정산 X";

            // 현재 보유 포인트 (정산 기준일 이후만 합산)
            $r_mb = sql_fetch("SELECT IFNULL(SUM(po_point),0) as s FROM {$g5['point_table']} WHERE mb_id='{$mb_id}' AND po_datetime >= '{$point_base_date}'");
            $mb_point = (int)$r_mb['s'];
            $po_mb_point = $mb_point + $point;

            // 포인트 이력 생성
            sql_query("
                INSERT INTO {$g5['point_table']}
                SET mb_id='{$mb_id}',
                    po_datetime='{$point_insert_day}',
                    po_content='".addslashes($content)."',
                    po_point='{$point}',
                    po_use_point=0,
                    po_mb_point='{$po_mb_point}',
            ...
            // 회원 테이블 포인트 갱신 (정산 기준일 이후 기준)
            sql_query("
                UPDATE {$g5['member_table']}
                SET mb_point = '{$po_mb_point}'
                WHERE mb_id  = '{$mb_id}'
            ");
        }

        /* =========================================================
           6. 해당 기간 포인트 정산완료 표시 (test_only=true 이면 스킵)
        ========================================================= */
        if(!$test_only){
            sql_query("
                UPDATE g5_point
            ...
            /* =========================================================
               7. 정산 완료된 상담내역 calc_flag='Y' 표시
            ========================================================= */
            sql_query("
                UPDATE thesaju_consulting
                SET calc_flag='Y'
                WHERE mb_id='{$mb_id}'
...
/**
 * 정산 내역 업데이트 (포인트 차감 없음)
 * g5_point_end의 price_free, price_paid, price_other, price_tot, vat_amount, withholding_tax, reply_fee 업데이트
 * mb_19=무료로열티%, mb_20=유료로열티% 기준
 * price_other: 상담 외 지급 포인트 (g5_point에서 c_no가 없는 건) → 유료로열티 적용
 *
 * @param string $mb_id   상담사 아이디
 * @param string $month   대상월 (예: '2026-02'), 미지정시 전월
 */
function set_con_account_v3($mb_id, $month = ''){
    ...
    try {
        ...
        // 대상월 계산
        $bmonth   = $month ? $month : date("Y-m", strtotime("-1 month"));
        $startday = $bmonth."-01 00:00:00";
        $endday   = date("Y-m-d", strtotime($bmonth."-01 +1 month"))." 00:00:00";
        ...
        // thesaju_consulting 집계 (환불 건 제외: DISCONNECT + usetm<30 + amt<=mb_4)
        // 포인트 미지급 건 제외: g5_point.c_no에 매칭되는 건만 집계
        $mb_4 = (int)$minfo['mb_4'];
        $con = sql_fetch("
            SELECT 
                IFNULL(SUM(tc.amt_free), 0) as amt_free,
        ...
        $withholding_tax = (int)floor($supply_price * 0.033);
        $reply_fee       = ($price_tot >= 50000) ? 20000 : 0;
        $price           = $supply_price - $withholding_tax - $reply_fee;


        // 기존 레코드 확인
        $row = sql_fetch("
            SELECT no FROM g5_point_end
            WHERE mb_id='{$mb_id}' AND month='{$bmonth}'
        ");
        ...
        if($row['no']){
            ...
            // INSERT (정산 레코드가 없는 경우)
            sql_query("
                INSERT INTO g5_point_end
                (mb_id, month, price, price_free, price_paid, price_other, price_tot, vat_amount, withholding_tax, reply_fee, amb_id, wr_datetime)
                VALUES
                ('{$mb_id}', '{$bmonth}', '{$price}', '{$price_free}', '{$price_paid}', '{$price_other}', '{$price_tot}', '{$vat_amount}', '{$withholding_tax}', '{$reply_fee}', 'system', NOW())
            ");
        }

        // 정산 완료된 상담내역 calc_flag='Y' 표시 (포인트 지급된 건만)
        sql_query("
            UPDATE thesaju_consulting tc
            SET tc.calc_flag='Y'
            WHERE tc.mb_id='{$mb_id}'
              AND tc.reason IN ('DISCONNECT','END_CHAT')
              AND tc.wr_datetime >= '{$startday}'
...
/////////// 상담사 보유금액 정산하기 ///
function set_con_account($mb_id){
    ...
    session_start();
    if (isset($_SESSION["executing_".$mb_id])) {// todo
        //if (isset($_SESSION["executing_".$mb_id]) && $mb_id != 'demonster1') {// todo

        die("중복정산");
    }

    $_SESSION["executing_".$mb_id] = true;
    $minfo    = get_member($mb_id);
    $bmonth   = date("Y-m",strtotime("-1 month")); // 매달 1일에 크론을 돌리므로, 전달을 기준으로 하여 1~말일동안의 상담수익을 계산함
    $startday = date("Y-m",strtotime("-1 month"))."-01 00:00:00";
    $endday   = date("Y-m",time())."-01 00:00:00";
    $lastday  = date('t', strtotime($bmonth."-01"));
    $point_insert_day = $bmonth."-".$lastday." 23:59:59";
    $price = 0;
    // po_content != '[수동] 11월 정산 중복' and po_content != '[수동]정산 시스템 오류로인한 미지급건'
    $sql = "select sum(po_point) as price from g5_point where  and mb_id='".$mb_id."' and po_datetime >= '".$startday."' and po_datetime <'".$endday."'";
    $row = sql_fetch($sql);
    $price = (int)$row["price"];//
    $sql = "select count(*) as ct from g5_point_end where mb_id='".$mb_id."' and month='".$bmonth."'";
    $row1 = sql_fetch($sql);
    $old_cnt = 0;
    $old_cnt = $row1["ct"];
    if(!$member["mb_id"]) $member["mb_id"] = "system";
    ...
    if($old_cnt <= 0){
        $isql = "insert into g5_point_end(`mb_id`, `month`, `price`, `amb_id`, `wr_datetime`) values('".$mb_id."', '".$bmonth."', '".$p_price."', '".$member["mb_id"]."', now())";
        $rs = sql_query($isql);
       ...
       // echo "성공5\n";//todo
          ...
          //  echo "성공6\n";//todo
            ...
            if($p_price > 0){
               ...
               // echo "성공7\n";//todo
                ...
                $msg = $bmonth."월 정기정산 10,000원 이하 정산 X"; //$bmonth. 안 들어가면 이미 처리된 것으로 인식함
            }

            //////////////////////////////////////////////////////////////////// 포인트 집어넣기 //////////////////////////////////////////////////////////////////////////////////////////
            //insert_point($mb_id, (-1)*$price, '월 정산', '@member', $mb_id, $msg);
            //$mb_id, $point, $content='', $rel_table='', $rel_id='', $rel_action='', $expire=0

            $point = (-1)*$price;
            $content = $bmonth.'월 정산'; //$bmonth. 들어가야 함
            $rel_table = '@member';
            $rel_id = $mb_id;
            $rel_action = $msg;
        ...
        if($rs){
            $usql = "update g5_point set p_end='Y' where mb_id='".$mb_id."' and po_datetime >= '".$startday."' and po_datetime <'".$endday."'";
            @sql_query($usql);
        }
    }
    // else{ // 필요없는 업데이트 구문 제거
    //     $usql = "update g5_point_end set price='".$p_price."', amb_id='".$member["mb_id"]."', up_datetime=now() where mb_id='".$mb_id."' and month='".$bmonth."'";
    //     sql_query($usql);
    // }
    unset($_SESSION["executing_".$mb_id]);

}
/////////// 상담사 보유금액 정산하기끝 ///

//20250728 eun 상담사 대기시간 구하기 작업 시작
/*function get_counselor_wait_time($mb_id) {
    //상담사가 현재 진행중인 상담이 있는지 확인한다.
    $sql = "SELECT * FROM g5_member
            WHERE csrid = '{$mb_id}' AND reason NOT IN ('DISCONNECT')
...
//20250728 eun 상담사 대기시간 구하기 작업 마감

function set_member_push_board($message, $link, $code, $id, $gubun=''){

    if($gubun=='')$gubun='1';
    //// 푸쉬 게시판에 넣는다 ////
    $query_memail_cnt = "";
    $query_memail_cnt = " insert into member_push set ";
    $query_memail_cnt .= " title = '".$message."', ";
    $query_memail_cnt .= " id = '".$id."', ";
    $query_memail_cnt .= " url = '".$link."', ";
    $query_memail_cnt .= " content = '".$message."', ";
    $query_memail_cnt .= " code = '".$code."', ";
    $query_memail_cnt .= " gubun = '".$gubun."', ";
    $query_memail_cnt .= " regdate = now()";

    //echo $query_memail_cnt;
    //echo "<br>";
    $result_memail_cnt = sql_query($query_memail_cnt);
    /// 푸쉬 게시판에 넣는다 끝 /////
}
...
/// 오늘에 내 운세 가져오기 ////
function get_today_my_fortune($mb_id){

    if(!$mb_id)return;

    $minfo = get_member($mb_id);

    if($minfo["mb_birth"])$yimsi = explode("-",$minfo["mb_birth"]);
    ...
    if($myear){
        $sql = "select * from g5_write_fortune where wr_1='".$today."' and wr_subject='".$myear."'";
        $row = sql_fetch($sql);
        return $row;
    }
}
...
/////// 예약 접속알람 보내기 //////////////
function set_resv_alrm($mb_id, $state){
    $sql = "select * from saju_resv where cs_id='".$mb_id."' order by regday desc";
    //echo $sql;
    $result = sql_query($sql);
    if($result){
        while($row=sql_fetch_array($result)){
            $hp = "";
            if($state=="IDLE" || $state=="RDVC" || $state=="RDCH"){ //20251205
                $hp = $row["mb_hp"];

                $cinfo = get_member($mb_id); /// 상담사 정보
                // 카톡 알림 발송 //
                ...
                $bizmsg->con_name           = $cinfo["mb_nick"];
                $rr = $bizmsg->send();
                // 카톡 알림 발송끝 //

                /// 카톡발송후 삭제 //
                $dsql = "delete from saju_resv where no='".$row["no"]."'";
                @sql_query($dsql);
                /// 카톡 발송후 삭제 끝 //
...
/// 회원 결제 건수 //
function get_member_pay_count($mb_id){

    if(!$mb_id)return;

    $sql = "select count(*) as cnt from saju_payment a left join g5_member b on(a.mb_id=b.mb_id) where (1) and ( (b.mb_id like '".$mb_id."%'))";
    $row = sql_fetch($sql);
...
/// 회원 누적 결제 금액 //
function get_member_pay_sum($mb_id){

    if(!$mb_id)return;
    $sql = "select sum(a.Amount) as price from saju_payment a left join g5_member b on(a.mb_id=b.mb_id) where (1) and ( (b.mb_id like '".$mb_id."%'))";
    //echo $sql;
...
/// 회원 상담 금액 가져오기 //
function get_con_pay_sum($mb_1, $mode='', $mb_hp=''){

    if(!$mb_1 && !$mb_hp) return;

    // 전화번호 정리 (하이픈 제거)
    $clean_phone = preg_replace('/[^0-9]/', '', $mb_hp);
    $clean_phone_esc = $clean_phone ? sql_escape_string($clean_phone) : '';

    $sql_mode = "";
    if($mode=="1"){
        $sql_mode .= " and preflag='Y' and reason !='START_CHAT'";
    }elseif($mode=="2"){
        $sql_mode .= " and preflag=''";
    }

    // membid 또는 전화번호(from)로 매칭
    $where_parts = [];
    if ($mb_1) {
        $where_parts[] = "membid = '".sql_escape_string($mb_1)."'";
    }
    if ($clean_phone_esc) {
        $where_parts[] = "REPLACE(`from`, '-', '') = '{$clean_phone_esc}'";
    }
    $where_member = implode(' OR ', $where_parts);

    $sql = "select sum(amt) as price from thesaju_consulting where (1) and ({$where_member}) {$sql_mode}";
...
/// 회원 가입 경과일 //
function get_join_dur($mb_id){
    if(!$mb_id)return;

    $sql = "select mb_datetime from g5_member where mb_id='".$mb_id."'";
    $row = sql_fetch($sql);

    $start_date = $row["mb_datetime"];
...
/// 사용 쿠폰 ///
function get_member_use_coupon($mb_id){
    if(!$mb_id)return;

    $sql = "select count(*) as cnt from g5_shop_coupon where mb_id='".$mb_id."'";
    $row = sql_fetch($sql);
    return $row["cnt"];
}
...
// 멤버 첨부된 파일을 얻는다. (배열로 반환)
function get_file2($mb_id)
{
    global $g5;
    ...
    {
        $no = $row['bf_no'];
        $file[$no]['href'] = G5_BBS_URL."/mb_download.php?mb_id=$mb_id&amp;no=$no";
        $file[$no]['download'] = $row['bf_download'];
        // 4.00.11 - 파일 path 추가
        $file[$no]['path'] = G5_DATA_URL.'/member2/'.$mb_id;
        $file[$no]['size'] = get_filesize($row['bf_filesize']);
        $file[$no]['datetime'] = $row['bf_datetime'];
        $file[$no]['source'] = addslashes($row['bf_source']);
...
function delete_member_thumbnail($mb_id, $file)
{
    ...
    $fn = preg_replace("/\.[^\.]+$/i", "", basename($file));
    $files = glob(G5_DATA_PATH.'/member2/'.$mb_id.'/thumb-'.$fn.'*');
    if (is_array($files)) {
        foreach ($files as $filename)
            unlink($filename);
    }
...
//========================= 작성하지 않은 상담기록 카운트 하기 ===============
function get_donot_my_c_history_cnt(){
    ...
    if($member["mb_level"]=="5"){
        $sql = "select no from thesaju_consulting where (1) and reason='DISCONNECT' and csrid='".$member["mb_1"]."'";
        //echo $sql;
        ...
        if($result){
            ...
            $s_cnt = 0;   // 상담 후기 갯수
            while($row=sql_fetch_array($result)){
                $sql1 = "select wr_id from g5_write_c_history where wr_10='".$row["no"]."'";
                $row1 = sql_fetch($sql1);
                if($row1["wr_id"]){
                    $s_cnt ++;
                }
...
//========================= 작성하지 않은 상담후기 카운트 하기 ===============
function get_donot_my_review_cnt(){
    ...
    if($member["mb_level"]=="2"){
        $sql = "select no from thesaju_consulting where (1) and reason='DISCONNECT' and membid='".$member["mb_1"]."'";
        //echo $sql;
        ...
        if($result){
            ...
            $s_cnt = 0;   // 상담 후기 갯수
            while($row=sql_fetch_array($result)){
                $sql1 = "select wr_id from g5_write_review where wr_10='".$row["no"]."'";
                $row1 = sql_fetch($sql1);
                if($row1["wr_id"]){
                    $s_cnt ++;
                }
...
function get_day_join_member_count($day){
    ...
    $sql = "select count(*) as cnt from g5_member where mb_datetime between '".$sday."' and '".$eday."' and mb_leave_date='' ";
    //echo $sql;
    //echo "<br><br>";
...
function get_day_down_coupon_count($day){
    if(!$day)return;
    ...
    $sql = "select count(*) as cnt from g5_shop_coupon where cp_datetime between '".$sday."' and '".$eday."'";
    //echo $sql;
    //echo "<br><br>";
    //exit;
    $row = sql_fetch($sql);
...
//// 상담사 상태값 저장
function set_constate($mb_id, $state){
    global $g5,$member;
    ...
    $sql = "select mb_id, status from member_status_history where mb_id='".$mb_id."' order by wr_datetime desc limit 0,1";
    ...
    if($rst){
        ...
        if(!$row["mb_id"]){
            $isql = "insert into member_status_history (`mb_id`,`status`,`wr_datetime`, `mmb_id`)values('".$mb_id."', '".$state."', now(), '".$member["mb_id"]."')";
            //echo $isql;
            //echo "<br>";
            @sql_query($isql);
        }else{
            if(trim($row["status"])!=trim($state)){
                $usql = "update member_status_history set status='".$state."', wr_datetime=now() where mb_id='".$mb_id."'";
                //echo $usql;
                //echo "<br>";
                @sql_query($usql);
            }
    ...
    //if(trim($row["status"])!=trim($state) && $state=="ABSE"){

    //$dsql = "delete from member_status_history where mb_id='".$mb_id."'";
    //@sql_query($dsql);
...
function set_crs_status_chg($crsid,$chg_status){ //상담사 csrid , 변경 코드상태값
    ...
    $fields = array(
        'cmd'    => 'csrstat',
        'csrid'  => (string)$crsid, // 고객 ID   test_c
...
function diff_time($start_date, $end_date) {
...
function set_join_member_coupon($mb_id){
    if(!$mb_id)return;

    $flag = false;

    $org_ids = "";
    $sql = "select * from g5_shop_coupon_zone where (cz_subject='회원가입 쿠폰' or cz_subject='회원가입쿠폰' or cz_id='39') order by cz_datetime desc limit 0,1";
    ...
    if($result){
        ...
        if($flag==false){
            ...
            $usql = "update g5_shop_coupon_zone set ids='".$uids."' where cz_id='".$row["cz_id"]."'";
            //echo $usql;
...
// 엠투넷에 등록되어있는 회원인지 조회한다//
function get_passcall_member($mb_1){
    $data = '{"list":[{"membid":"'.$mb_1.'"}]}';
    $murl1 = "memb-mgr";
    $jresult1 = send_mjson($murl1, $data, 'GET');
    //print_r($jresult1);
    if($jresult1["req_result"]=="00"){ /// 회원 조회 성공
        return true;
    }else{ /// 등록실패
        return false;
    }
// 엠투넷에 등록되어있는 회원인지 조회한다 끝//
}

////////////////////////// 월 정산 //////////////////////////////////////////////////////
function month_pay_execute(){

    $nowday = date("d",time());
    $month = date("Y-m",strtotime("-1 month"));

    if($nowday=="01" || $nowday=="1"){
        $sql1 = "select count(*) as ct from g5_point_end where month='".$month."'";
        $row1=sql_fetch($sql1);
        if($row1["ct"] <=0){
...
function member_auto_pay_card($mb_id){
    global $member, $g5;

    if(!$mb_id)return;
    $sql = "select * from g5_member_auto_pay where mb_id='".$mb_id."'";
    $row = sql_fetch($sql);
    if($row["mb_id"]){
        return $row;
    }
...
function maskFlexible($string, $startPercent = 30, $maskPercent = 50, $maskChar = '*') {
    $length = strlen($string);
    $start = floor($length * ($startPercent / 100));
    $maskLength = floor($length * ($maskPercent / 100));

    $maskedPart = str_repeat($maskChar, $maskLength);
    return substr_replace($string, $maskedPart, $start, $maskLength);
}


/*function chg_point_pay($amount){
    ...
    }elseif($amount=="110000"){
        $coin = (100000*0.12)+100000;
    }elseif($amount=="220000"){
        $coin = (200000*0.12)+200000;
    }elseif($amount=="330000"){
...
/**
 * 결제 금액(VAT포함) → 실제 충전 포인트 계산 함수
 *
 * @param int|string $amount  결제 금액 (예: 33000, 55000 등)
 * @return int  지급될 총 포인트 (보너스 포함)
 */
function chg_point_pay($amount) {
    if (!$amount) return 0;
    $amount = (int)$amount;
    ...
    // DB에서 계층별 금액 정보 불러오기
    $sql = "SELECT price, (price * 1.1) AS pay_amount, total_point
              FROM account_config
             ORDER BY product_id";
    $res = sql_query($sql);

    $coin = $amount; // 기본값: 결제금액 그대로

    // DB 데이터와 결제금액 매칭
    while ($row = sql_fetch_array($res)) {
        // DB의 pay_amount(=VAT포함금액)가 실제 결제금액과 일치할 경우
        if ((int)round($row['pay_amount']) === $amount) {
            $coin = (int)$row['total_point']; // 총 포인트 반환
            break;
        }
    }
...
// 암호화//$endata = @openssl_encrypt($data , "aes-128-cbc", $crypt_pass, true, $crypt_iv);
// $endata = base64_encode($endata);
// echo "ENCODE DATA : " . $endata . "\n";
// 복호화//$data = base64_decode($endata);
// $endata = @openssl_decrypt($data, "aes-128-cbc", $crypt_pass, true, $crypt_iv);
// echo "DECODE DATA : " . $endata . "\n";

function aligo_send_coupon_save($cp_id, $cz_id, $mb_id_token = '',$set_type = '1'){
        ...
        if (empty($mb_id_token)) return array('total' => 0, 'rows' => 0);

        // 쿠폰의 정보를 가져옴
        $cz_row = sql_fetch("SELECT * FROM g5_shop_coupon_zone WHERE cz_id = '".addslashes($cz_id)."'");
        ...
        if ($mb_id_token === 'all') {
            ...
            $offset = 0;
            while (true) {
                $result = sql_query("SELECT mb_no, mb_id, mb_name, mb_nick, mb_hp FROM g5_member WHERE mb_level = '2' AND mb_leave_date = '' LIMIT {$offset}, {$chunk_size}");
                $cnt = 0;
                while ($mb_row = sql_fetch_array($result)) {
                    $cnt++;
                    $total++;
                    $chunk[] = array(
                        'mb_id'      => $mb_row['mb_id'],
                        'phn'        => $mb_row['mb_hp'],
                        'usr_name'   => $mb_row['mb_name'],
                        'usr_nick'   => $mb_row['mb_nick'],
                        'cp_id'      => $cp_id,
                        'cp_name'    => $cz_row['cz_subject'],
                        'cp_end_data'=> $cz_row['cz_end'],
                        'cz_subject' => $at_type,
                        'ct_start'   => $cz_row['cz_start'],
                        'ct_end'     => $cz_row['cz_end'],
                        'cz_point'   => $cz_row['cz_point'],
                        'cz_period'  => $cz_row['cz_period'],
                        'set_type'   => $set_type
                    );
                    if (count($chunk) >= $chunk_size) {
                        $json_data = json_encode($chunk, JSON_UNESCAPED_UNICODE);
                        $isql = "INSERT INTO aligo_send_all_t (aligo_subject, aligo_data, aligo_url, send_status, set_type, aligo_wdate)
                                 VALUES ('".addslashes($at_type)."', '".addslashes($json_data)."', 'shop/coupon.php', 'N', '".$set_type."', NOW())";
                        sql_query($isql);
                        $row_cnt++;
                        $chunk = array();
                    }
                }
                if ($cnt < $chunk_size) break;
                $offset += $chunk_size;
            }
            if (count($chunk) > 0) {
                $json_data = json_encode($chunk, JSON_UNESCAPED_UNICODE);
            ...
            $set_type = "2";
            $chunk_data_list = array();
            // 토큰 → 배열 변환
            $mb_ids = array_filter(array_map('trim', explode(',', $mb_id_token)));
            if (empty($mb_ids)) return array('total' => 0, 'rows' => 0);

            // 500개씩 끊어서 IN 쿼리로 회원 조회
            $mb_chunks = array_chunk($mb_ids, $chunk_size);
            foreach ($mb_chunks as $mb_batch) {
                $in_ids = implode("','", array_map('addslashes', $mb_batch));
                $result = sql_query("SELECT mb_no, mb_id, mb_name, mb_nick, mb_hp FROM g5_member WHERE mb_id IN ('{$in_ids}')");
                while ($mb_row = sql_fetch_array($result)) {
                    $total++;
                    $item = array(
                        'mb_id'      => $mb_row['mb_id'],
                    ...
                    $chunk[] = $item;
                    $chunk_data_list[] = $item;
                    if (count($chunk) >= $chunk_size) {
                        $json_data = json_encode($chunk, JSON_UNESCAPED_UNICODE);
                        $isql = "INSERT INTO 
                                    aligo_send_all_t (aligo_subject, aligo_data,aligo_url,send_status, set_type,aligo_wdate)
                                 VALUES ('".addslashes($at_type)."', '".addslashes($json_data)."','shop/coupon.php', 'Y', '".$set_type."', NOW())";
                        sql_query($isql);
                        $row_cnt++;
                        $chunk = array();
                    }
            ...
            // set_type=2(특정회원)는 INSERT 직후 바로 전송 (방금 넣은 데이터만)
            aligo_send_coupon_flush($chunk_data_list);
        }

        return array('total' => $total, 'rows' => $row_cnt);
}


/**
 * 방금 INSERT한 recv_list만 받아서 바로 알림톡 전송
 * @param array $recv_list - 수신자 배열
 */
function aligo_send_coupon_flush($recv_list = array()){

    if (empty($recv_list) || !is_array($recv_list)) return 0;

    include_once(G5_PLUGIN_PATH.'/wz_alimtalk_bizm/config.php');
    include_once(G5_PLUGIN_PATH.'/wz_alimtalk_bizm/bizmsg.class.php');

    $bizmsg = new bizmsg();
    $bizmsg->at_type = '쿠폰발송알림';

    // 100건씩 나눠서 전송
    $chunks = array_chunk($recv_list, 100);
    foreach ($chunks as $chunk) {
        $bizmsg->send_muti($chunk);
    }

    return count($recv_list);
}
