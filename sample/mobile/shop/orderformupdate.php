...
else if ($od_settle_case == "신용카드")
{
    ...
    if($od_misu == 0){
		...
		$bizmsg = new bizmsg();
			$bizmsg->phn                = $member["mb_hp"];
			$bizmsg->at_type            = '입금확인'; //
			$bizmsg->usr_name     = $member["mb_name"];
			$bizmsg->goods_name           = $it_names;  // 상품명
    ...
    if($is_sms_send) {
		...
		// 카톡 알림 발송 //
			...
			$bizmsg = new bizmsg();
				...
				$bizmsg->usr_name     = $member["mb_name"];
				$bizmsg->goods_name           = $it_names;  // 상품명
				$bizmsg->usr_bank_price           = number_format($od_misu);  /// 입금액
				$bizmsg->usr_bank_num           = $od_bank_account;  /// 입금계좌
				$rr = $bizmsg->send(); 
		   // 카톡 알림 발송끝 //
