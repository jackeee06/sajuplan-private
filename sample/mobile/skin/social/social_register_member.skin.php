...
<div class="social_register">
    ...
    <form name="fregisterform" id="fregisterform" action="<?php echo $register_action_url; ?>" onsubmit="return fregisterform_submit(this);" method="POST" autocomplete="off">
        ...
        <input type="hidden" id="hp_auth" name="hp_auth" value=""/>
        <input type="hidden" id="a_num"   name="a_num"   value=""/>

        <p><i class="fa fa-check-circle" aria-hidden="true" style="margin-bottom: 14px;"></i> 회원가입약관 및 개인정보 수집 및 이용의 내용에 동의하셔야 회원가입 하실 수 있습니다.</p>
            ...
            <div class="tbl_head01 tbl_wrap">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="2">고객서비스 이용에 관한 통지,<br>CS 대응을 위한 이용자 식별</td>
                        </tr>
                        <tr>
        ...
        <div id="register_form" class="form_01">
            ...
            <ul>
                ...
                <li style="display:none;">
                    ...
                    <input type="text" name="mb_email" value="<?php echo isset($user_email) ? $user_email : ''; ?>" id="reg_mb_email" <?php echo (isset($user_email) && $user_email != '' && !$is_exists_email)? "readonly":''; ?> class="frm_input email full_input" size="70" maxlength="100" placeholder="E-mail (필수)">
                    <div class="check"><?php echo $email_msg; ?></div>
                </li>

                <li style="display: flex; align-items: center; gap: 8px;">
                    <label for="input_mb_hp" class="sound_only">휴대폰번호 (필수)</label>
                    <input type="text" name="mb_hp" value="<?php echo get_text($member['mb_hp']) ?>" id="input_mb_hp" <?php echo ($config['cf_req_hp'])?"required":""; ?> class="frm_input  full_input <?php echo ($config['cf_req_hp'])?"required":""; ?>" maxlength="20" placeholder="휴대폰번호 (ex : 010-0000-0000)">
                    <button type="button" class="btn_frmline btn" id="win_hp_auth" style="white-space:nowrap;" onclick="">휴대폰 인증</button>
                </li>
                <script>
                    <?php if($config['cf_auth_hp']) { ?>
                    // 휴대폰 번호를 입력해주세요.
                        $("#win_hp_auth").click(function() {
                            var reg_mb_hp = $("#input_mb_hp").val();
                            if(reg_mb_hp.trim() == ""){
                                webAlert("휴대폰번호를 입력해주세요.");
                                return false;
                            }
                            opt = 'scrollbars=0,width=400,height=320,top=10,left=20';
                            phone_auth_window(g5_bbs_url+"/sms_v2.php?mb_hp="+$("#input_mb_hp").val(), "sms_auth", opt);
                        });
                    <?php } ?>                    
                </script>
                <!-- 유입경로 추가 -->
                ...
                <!-- 성별 선택 -->
                <li>
                    <label class="sound_only">성별 (필수)</label>
                    <div style="display:flex; align-items:center; gap:16px; padding:10px 0;">
                        <span style="font-size:14px; font-weight:bold;">성별</span>
                        <label for="mb_10_1" style="display:flex; align-items:center; gap:4px; cursor:pointer; color:#333; font-weight:500;">
                            <input type="radio" id="mb_10_1" name="mb_10" value="남" required> 남자
                        </label>
                        <label for="mb_10_2" style="display:flex; align-items:center; gap:4px; cursor:pointer; color:#333; font-weight:500;">
                            <input type="radio" id="mb_10_2" name="mb_10" value="여" required> 여자
                        </label>
                    </div>
                </li>
                <!-- 생년월일 선택 -->
                <li>
                    <label for="mb_birth" class="sound_only">생년월일 (필수)</label>
                        <div style="display:flex; align-items:center; gap:8px;">
                            <input type="date" name="mb_birth" value="" id="mb_birth" required class="frm_input full_input" placeholder="생년월일" style="flex:1;">
                            <div style="display:flex; align-items:center; justify-content:center; gap:8px; white-space:nowrap; margin-top:18px;">
                                <label for="mb_11_1" style="display:flex; align-items:center; gap:4px; cursor:pointer; color:#333; font-weight:500;">
                                    <input type="radio" id="mb_11_1" name="mb_11" value="양력" checked required> 양력
                                </label>
                                <label for="mb_11_2" style="display:flex; align-items:center; gap:4px; cursor:pointer; color:#333; font-weight:500;">
                                    <input type="radio" id="mb_11_2" name="mb_11" value="음력" required> 음력
                                </label>
                            </div>
                        </div>
                </li>
                ...
                <!-- 유입경로 -->
                <li>
                    <label for="" style="font-size:14px; font-weight:bold; display:block; margin-bottom:8px; color:#333;">유입경로</label>
                    <textarea 
                      name="mb_21" 
                      id="mb_21" 
                      class="frm_input full_input" 
                      rows="3" 
                      style="width:100%; resize:vertical; color:#333;" 
                      placeholder="어떤 경로로 저희 서비스를 알게 되셨나요? (예: 지인추천, 검색, SNS 등)"></textarea>
                    <style>
                        #mb_12::placeholder { color: #666; opacity: 1; }
                    </style>
                </li>
        ...
        <div class="btn_confirm">
            <a style="border-radius: 6px;" href="<?php echo G5_URL ?>" class="btn_cancel">취소</a>
    ...
    <div class="connect-fg">
        <form method="post" action="<?php echo $login_action_url ?>" onsubmit="return social_obj.flogin_submit(this);">
            ...
            <div class="connect-desc">
                ...
                이후 SNS 아이디로 로그인 하시면 기존 아이디로 로그인 할 수 있습니다.
            </div>
...
<script>
    // 휴대폰 번호 자동 하이픈(모바일 3-3-4 또는 3-4-4 대응)
    function formatKRMobile(raw) {
        let d = String(raw).replace(/\D/g, '').slice(0, 11); // 숫자만, 최대 11자리
    ...
    // 정규식: 010/011/016/017/018/019 + (3 or 4) + 4, 반드시 하이픈 포함
    const HP_PATTERN = /^01[016789]-(\d{3}|\d{4})-\d{4}$/;

    // 페이지 로드 후 바인딩
    jQuery(function($){
        const $hp = $('#mb_hp, input[name="mb_hp"]');
        ...
        $hp.attr({
            ...
            title: '휴대전화번호는 010-1234-5678 형태로 입력하세요.'
        });

        // 타이핑/붙여넣기 시 자동 하이픈
        $(document).on('input', '#mb_hp, input[name="mb_hp"]', function(){
            this.value = formatKRMobile(this.value);
        });

        // 제출 시 검증(페이지 내 모든 폼에 대해—필요시 특정 폼 셀렉터로 좁히기)
        /*$(document).on('submit', 'form', function(e){
            const $field = $(this).find('#mb_hp, input[name="mb_hp"]');
            if ($field.length) {
                const v = $field.val().trim();
                if (!HP_PATTERN.test(v)) {
                    alert('휴대전화번호는 반드시 하이픈(-) 포함하여 010-1234-5678 형태로 입력해주세요.');
                    $field.focus();
                    e.preventDefault();
                }
            }
