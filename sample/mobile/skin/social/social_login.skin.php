...
<style>
.sns-login-title { position:relative; margin-bottom:20px;}
.sns-login-title:after { content:''; width:100%; height:1px; background-color:#ddd; position:absolute; top:50%; left:0; z-index:0;}
.sns-login-title span { display:inline-block; font-size:14px; color:#999; padding:0 20px; position:relative; z-index:1; background-color:#fff;}
</style>

<div class="login-sns sns-wrap-32 sns-wrap-over" id="sns_login">
    <!--<h3>소셜계정으로 로그인</h3>-->
    <div class="sns-login-title"><span>1초만에 간단하게 로그인해보세요!</span></div>
    <div class="sns-wrap">
        <?php if( social_service_check('naver') ) {     //네이버 로그인을 사용한다면 ?>
        <?php
        // https://sajumoon.co.kr/plugin/social/popup.php
        ?>
