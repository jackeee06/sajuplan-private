...
<label class="frm_label">SNS 로그인</label>
<li>
    ...
    <div class="reg-form sns-wrap-reg">
        ...
        <?php 
        // mb_id 기반으로 소셜 provider 자동 추가 (social_profiles 테이블에 없는 경우 대비)
        // 해당부분만 추가 되었음////////////////////////////////////////////////////////////////////////////
        if(isset($member['mb_id']) && is_array($my_provides)){
            $check_mb_id = strtolower($member['mb_id']);
            if(strpos($check_mb_id, 'kakao_') === 0 && !in_array('kakao', $my_provides)){
                $my_provides[] = 'kakao';
            } else if(strpos($check_mb_id, 'naver_') === 0 && !in_array('naver', $my_provides)){
                $my_provides[] = 'naver';
            } else if(strpos($check_mb_id, 'google_') === 0 && !in_array('google', $my_provides)){
                $my_provides[] = 'google';
            } else if(strpos($check_mb_id, 'apple_') === 0 && !in_array('apple', $my_provides)){
                $my_provides[] = 'apple';
            }
        }
        ///////////////////////////////////////////////////////////////////////////////////////////////
...
<style>
.social_guide { font-size:13px; color:#888;<hr />
}
</style>
