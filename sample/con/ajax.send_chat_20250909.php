...
//include_once __DIR__ . '/../lib/aligo_sms.lib.php'; // 형제 폴더
...
//20250822 eun mtonet에 request했다는 기록 남기기 시작
// 요청/응답 기록 테이블: request_consulting_t
function insert_request_consulting_log(array $p)
{
    $esc = function ($v) {
        return isset($v) ? "'" . sql_real_escape_string(is_scalar($v) ? (string)$v : json_encode($v, JSON_UNESCAPED_UNICODE)) . "'" : "NULL";
    };
...
insert_request_consulting_log([
  ...
  //  'http_status' => $aligo_res['_http_code'] ?? null,  운영서버에서는 이거 사용하기         // send_ch_json 내부에서만 알 수 있음(수정 가능하면 거기서 기록)
    'http_status' => null, // 알리고/비즈엠 비활성화: 외부 문자발송 HTTP 상태 기록 안 함
    'cmd' => 'csrchat',
    'url' => $log_url,
    'membid' => $memid,
    'csrid' => $csrid,
...
if ($jresult["req_result"] == "00") { /// 성공
  ...
  //$rr = $bizmsg->send(); //TODO 해당 부분 실제로는 주석 풀기
   ...
   // $aligo_res = aligo_send_and_parse($csr_row['mb_hp'], $sms_msg, $sms_subject);
    ...
    //var_dump($jresult);
    //exit;
    echo json_encode(['result' => false, 'msg' => $jresult["resultmessage"]]);
}
