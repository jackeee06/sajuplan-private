...
// 2차 검증작업시작 - sms_auth 테이블에서 인증번호 서버측 검증 (신규가입만)
if ($w == '') {
...
if($result) {
    ...
    if($jresult1["req_result"]=="00"){ /// 등록성공
        ...
        if($jresult1["membid"]){ /// 회원번호 업데이트
            ...
            $usql = "update {$g5['member_table']} set mb_1='".$jresult1["membid"]."' where mb_id='".$mb_id."'";
        ...
        alert('회원등록이 실패하였습니다. 이유: '.$jresult1["resultmessage"],'/');
