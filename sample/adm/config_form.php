...
if(!$config['cf_1_subj']) $config['cf_1_subj'] = "후기 작성 완료시 포인트";
if(!isset($config['cf_1'])) $config['cf_1'] = 0;
if(!$config['cf_2_subj']) $config['cf_2_subj'] = "후기 포인트 지급 (사용코인제한)";
if(!isset($config['cf_2'])) $config['cf_2'] = 0;
...
// ADD 휴대폰 인증 추가 
if(!isset($config['cf_auth_hp'])) {
    sql_query("ALTER TABLE `{$g5['config_table']}`
                ADD `cf_auth_hp` tinyint(4) NOT NULL DEFAULT 0
    ", true);
}
if(!sql_query(" DESC sms_auth ", false)) {
    sql_query(" CREATE TABLE IF NOT EXISTS `sms_auth` (
  `a_idx` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `a_hp` varchar(20) NOT NULL,
  `a_auth` int(10) NOT NULL DEFAULT 0,
  `a_num` varchar(4) NOT NULL DEFAULT '',
...
<section id="anc_cf_basic">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <th scope="row"><label for="cf_1">후기 작성 완료시 포인트</label></th>
            <td>
                <input type="text" name="cf_1" value="<?php echo (int)$config['cf_1'] ?>" id="cf_1" class="frm_input" size="5"> 점
            </td>
        </tr>

        <tr>
            <th scope="row"><label for="">후기 지급 포인트 (코인사용제한)</label></th>
            <td>
                <input type="text" name="cf_2" value="<?php echo (int)$config['cf_2'] ?>" id="cf_2" class="frm_input" size="10"> 점
            </td>
            <th></th>
            <td></td>
        </tr>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <td>
				...
				<!-- ADD -->
				<input type="checkbox" name="cf_auth_hp" value="1" id="cf_auth_hp" <?php echo $config['cf_auth_hp']?'checked':''; ?>> <label for="cf_req_hp">휴대폰 인증</label>
...
<section id="anc_cf_extra">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tbody>
        <?php for ($i=3; $i<=10; $i++) { if ($i == 1) continue; ?>
        <tr>
