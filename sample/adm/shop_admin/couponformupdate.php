...
if($w == '') {
    ...
    // 삭제도안하고 그냥 수정을 치네; (여러명 등록은 빡세겠는데?)
    // 이미 사용한 쿠폰 외에는 쿠폰에서 삭제처리후 등록 처리가 필요함.
...
if($w == '' && ($_POST['cp_sms_send'] || $_POST['cp_email_send'])) {
    ...
    for($i=0; $i<$count; $i++) {
		...
		/// 만약 포인트추가 쿠폰이면 바로 적용한다.
		if($_POST["cp_method"]=="4"){

				$csql = "select * from g5_shop_coupon_zone where cz_id='".$cp_id."'";
				//echo $csql;
				//echo "<br>";
				$crow = sql_fetch($csql);
				$point = 0;
				if($crow["cz_id"]){
					$cz_point = $crow["cz_point"];
				}

				insert_point($arr_send_list[$i]['mb_id'], (int)$cz_point, "쿠폰 $cp_id 발급 및 사용");
				/// 포인트 증가 테이블은 다운로드 즉시 사용됨
				$sql2 = " insert into {$g5['g5_shop_coupon_log_table']}
								set cp_id       = '{$cp_id}',
									mb_id       = '".$arr_send_list[$i]['mb_id']."',
									od_id       = '',
									cp_price    = '{$cp_price}',
