...
function find_banner_asset($bn_id) {
    ...
    foreach (['webp','jpg','jpeg','png','gif','bmp','mp4'] as $ext) {
        $p = "{$dir}/{$bn_id}.{$ext}";
        if (is_file($p)) {
            return [$p, "{$url}/{$bn_id}.{$ext}", $ext];
        }
    }
    // 레거시(무확장자) 호환
    $legacy = "{$dir}/{$bn_id}";
    if (is_file($legacy)) {
        return [$legacy, "{$url}/{$bn_id}", ''];
    }
    return [null, null, null];
}
//20250915 webp 파일 업로드 작업 마감
    ...
    <div class="local_ov01 local_ov">
        ...
        <form name="flist" class="local_sch01 local_sch">
            ...
            <select name="bn_position" id="bn_position">
                ...
                <option value="회원가입완료"<?php echo get_selected($bn_position, '회원가입완료', true); ?>>회원가입완료</option>

                <option value="메인-비주얼"<?php echo get_selected($bn_position, '메인-비주얼', true); ?>>메인-비주얼</option>

                <option value="메인-상단띠배너"<?php echo get_selected($bn_position, '메인-상단띠배너', true); ?>>메인-상단띠배너</option>

                <option value="메인-중앙배너"<?php echo get_selected($bn_position, '메인-중앙배너', true); ?>>메인-중앙배너</option>

                <option value="로그인-상단띠배너"<?php echo get_selected($bn_position, '로그인-상단띠배너', true); ?>>로그인-상단띠배너</option>

                <option value="마이페이지"<?php echo get_selected($bn_position, '마이페이지', true); ?>>마이페이지</option>

                <option value="일반-상담후기"<?php echo get_selected($bn_position, '일반-상담후기', true); ?>>일반-상담후기</option>

                <option value="일반-이용안내"<?php echo get_selected($bn_position, '일반-이용안내', true); ?>>일반-이용안내</option>

                <option value="일반-상담사신청"<?php echo get_selected($bn_position, '일반-상담사신청', true); ?>>일반-상담사신청</option>

                <option value="상담사-코인내역"<?php echo get_selected($bn_position, '상담사-코인내역', true); ?>>상담사-정산내역</option>

                <option value="상담사-공지사항"<?php echo get_selected($bn_position, '상담사-공지사항', true); ?>>상담사-공지사항</option>

                <option value="이벤트1"<?php echo get_selected($bn_position, '이벤트1', true); ?>>이벤트1</option>

                <option value="이벤트2"<?php echo get_selected($bn_position, '이벤트2', true); ?>>이벤트2</option>

                <option value="이벤트3"<?php echo get_selected($bn_position, '이벤트3', true); ?>>이벤트3</option>

                <option value="오늘의운세"<?php echo get_selected($bn_position, '오늘의운세', true); ?>>오늘의운세</option>

                <option value="소원다락방-상단"<?php echo get_selected($bn_position, '소원다락방-상단', true); ?>>소원다락방-상단</option>

                <option value="소원다락방-하단"<?php echo get_selected($bn_position, '소원다락방-하단', true); ?>>소원다락방-하단</option>
        ...
        <table>
          ...
          limit $from_record, $rows  ";
            ...
            for ($i=0; $row=sql_fetch_array($result); $i++) {
                ...
                // 20250716 eun 배너에도 mp4 넣을 수 있도록 작업 시작
                /*  $bimg = G5_DATA_PATH.'/banner/'.$row['bn_id'];

                  if(file_exists($bimg)) {
                      //파일 mime 타입 확인
                      $mime = mime_content_type($bimg);
                      if ($mime == 'video/mp4') {
                          $bn_img = '';
                          // $bn_img .= '<video src="'.G5_DATA_URL.'/banner/'.$row['bn_id'].'controls muted loop playsinline;. alt="'.get_text($row['bn_alt']).'">'.'</video>';
                          $bn_img = '<video'
                              . ' src="' . G5_DATA_URL . '/banner/' . $row['bn_id'] . '"'
...
// 미리보기(이미지/동영상)
                ...
                if ($path) {
                    // 캐시 버스트(기존 bn_time 숫자 사용, 없으면 현재시각)
                    $v = preg_replace('/[^0-9]/', '', $row['bn_time'] ?? '') ?: time();
                    $alt = get_text($row['bn_alt']);

                    if ($ext === 'mp4') {
                        // 동영상 미리보기
                        $bn_img = '<video controls preload="metadata" '.
                            'style="max-width:800px;width:100%;height:auto;display:block">'.
                            '<source src="'.$src.'?'.$v.'" type="video/mp4">'.
                            '이 브라우저는 비디오를 지원하지 않습니다.'.
                            '</video>';
                    } else {
                        // 이미지(WebP 포함) 미리보기 — getimagesize 없이 바로 출력
                        $bn_img = '<img src="'.$src.'?'.$v.'" alt="'.$alt.'" '.
                ...
                $bg = 'bg'.($i%2);
                // 20250716 eun 배너에도 mp4 넣을 수 있도록 작업 마감
                ...
                <tr class="<?php echo $bg; ?>">
                    ...
                    <td headers="th_img" colspan="7" class="td_img_view sbn_img">
                        ...
                        <div class="sbn_image" style="display:block !important;"><?php echo $bn_img; ?></div>
