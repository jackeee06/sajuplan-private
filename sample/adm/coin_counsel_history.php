...
$where_base = " where (1) and csrid !='' "; // 검색/기간 등 공통
            ...
            <div class="tbl_head01 tbl_wrap">
                <table>
                    ...
                    <tr>
                        ...
                        <!--AG9 확인을 위해서 개발 시 상담사와 사용자의 mtonet id 표시-->
                        <?php  if($_SERVER['REMOTE_ADDR'] == "115.93.39.5"){ ?>
                            <th scope="col" id="mb_list_mng">개발상담사</th>
                        <?php } else { ?>  <th scope="col" id="mb_list_auth" class="">분야</th>
                        <? } ?>
                        <th scope="col" id="mb_list_mng">상담시작</th>
                        <th scope="col" id="mb_list_mng">상담종료</th>
                        <th scope="col" id="mb_list_mng">진행시간</th>
                        <?php  if($_SERVER['REMOTE_ADDR'] == "115.93.39.5"){ ?>
                            <th scope="col" id="mb_list_mng">개발사용자memb</th>
                        <?php } else { ?> <th scope="col" id="mb_list_mng">유·무료</th> <? } ?>
                        <!--AG9 확인을 위해서 개발 시 상담사와 사용자의 mtonet id 표시-->
                    ...
                    <tbody>
                    <?php
                    function format_kor_phone($v){
                        $n = preg_replace('/\D+/', '', (string)$v);
                        if ($n === '') return '';

                        // 서울(02) 지역번호
                        if (strpos($n, '02') === 0) {
                            if (strlen($n) === 9)  return preg_replace('/^(02)(\d{3})(\d{4})$/', '$1-$2-$3', $n);  // 02-###-####
                            if (strlen($n) === 10) return preg_replace('/^(02)(\d{4})(\d{4})$/', '$1-$2-$3', $n);  // 02-####-####
                        }

                        // 휴대폰/일반지역 3자리 국번
                        if (strlen($n) === 11) return preg_replace('/^(\d{3})(\d{4})(\d{4})$/', '$1-$2-$3', $n);   // 010-####-####
                        if (strlen($n) === 10) return preg_replace('/^(\d{3})(\d{3})(\d{4})$/', '$1-$2-$3', $n);   // 011-###-#### 등

                        // 내선(8자리 등)
                        if (strlen($n) === 8)  return preg_replace('/^(\d{4})(\d{4})$/', '$1-$2', $n);            // ####-####
                    ...
                    for ($i=0; $row=sql_fetch_array($result); $i++) {
                        ...
                        $bind_member = null;
                        if (empty($row["membid"])) {
                            $bind_phone = preg_replace('/[^0-9]/', '', $row['from'] ?? '');
                            if ($bind_phone !== ''){
                                $bind_phone_esc = sql_escape_string($bind_phone);
                                $sql_bind = "select mb_id, mb_name, mb_hp from g5_member where REPLACE(mb_hp, '-', '') = '{$bind_phone_esc}' limit 1";
                                $bind_member = sql_fetch($sql_bind);
                            }
                        }
                        ?>
                        <tr class="<?php echo $bg; ?>">
                            ...
                            <td headers="mb_list_grp" class="">
                                ...
                                if (!empty($row["membid"])) {
                                    echo $minfo["mb_name"];
                                } elseif (!empty($bind_member["mb_name"])) {
                                    echo $bind_member["mb_name"];
                                } else {
                                    echo htmlspecialchars(format_kor_phone($row['from']), ENT_QUOTES, 'UTF-8');
                                }
                                ?>
                            </td>
                            ...
                            <td headers="mb_list_grp" class=""> <?php   if($_SERVER['REMOTE_ADDR'] == "115.93.39.5") {
                                    echo ($row["csrid"]) ;
                                }
                                else {
                                    echo ($coninfo["ca_name"]);
                                }?></td>
                            ...
                            <td headers="mb_list_auth" class="">
                                ...
                                echo ($amt <= $cinfo['mb_4'] && $usetm < 30 && !$is_chat) ? '0(환불)' : number_format($amt);  //917 환불
