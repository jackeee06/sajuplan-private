...
// 차단회원수 당월
$sql = " select count(*) as cnt {$sql_common} {$sql_search} and mb_intercept_date <> '' nd  mb_datetime >= '".$nowdays."' ";
$row = sql_fetch($sql);
...
<style>
.container_wr { background-color:#fafafa;}
...
.index_dash dl { padding:10px 12px; background-color:#fafafa; margin: 0 0 10px; border-radius:8px;}
...
<section class="index w50 pink_bo" style="">   
    <div class="">
    <h2>
    	신규충전
        <a href="../adm/coin_pay_history.php"><span class="index_more pink_bg">더보기 <i class="xi-angle-right"></i></span></a>  
...
<section class="index black_bo">
    ...
    <div class="index_dash index_flex">
    	...
    	<dl class="w25">
            ...
            <dd class="w100 dot"><span class="f_700">상담중 <?=number_format(get_conn_state_count('CONN', '신점'))?>명</span></dd>
            <dd class="w100 dot">상담가능 <?=number_format(get_conn_state_count('IDLE', '신점'))?>명</dd>
            <dd class="w100 dot">부재중 <?=number_format(get_conn_state_count('ABSE', '신점'))?>명</dd>
        </dl>
    	<dl class="w25">
        	<dt><span class="icon_cate saju">사주</span></dt>
            <dd><span class=" f_800"><?=number_format($saju_count)?></span>명</dd>
            <dd class="w100 dot"><span class="f_700">상담중 <?=number_format(get_conn_state_count('CONN', '사주'))?>명</span></dd>
            <dd class="w100 dot">상담가능 <?=number_format(get_conn_state_count('IDLE', '사주'))?>명</dd>
            <dd class="w100 dot">부재중 <?=number_format(get_conn_state_count('ABSE', '사주'))?>명</dd>
        </dl>
    	<dl class="w25">
        	<dt><span class="icon_cate simli">심리</span></dt>
            <dd><span class=" f_800"><?=number_format($sim_count)?></span>명</dd>
           <dd class="w100 dot"><span class="f_700">상담중 <?=number_format(get_conn_state_count('CONN', '심리'))?>명</span></dd>
            <dd class="w100 dot">상담가능 <?=number_format(get_conn_state_count('IDLE', '심리'))?>명</dd>
            <dd class="w100 dot">부재중 <?=number_format(get_conn_state_count('ABSE', '심리'))?>명</dd>
        </dl>
    </div>
</section>
    ...
    <div class="index_dash index_flex">
        ...
        <div class="w33">
        	...
        	<div class="tbl_head01 tbl_wrap">
        		...
        		<tr>
		            <td class=""><?=$minfo["mb_name"]?> ( <?=$res["membid"]?> )</td>
