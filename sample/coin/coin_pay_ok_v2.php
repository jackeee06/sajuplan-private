<?php 
	...
	if($req_result == "0000"){ //		'//오류가 아닐때..
		...
		// 이미 결제 완료된 건이 있는지 확인 (od_status='0'인 대기 건은 제외)
		$sql = "select count(*) as ct from saju_payment where Oid='".$oid."' and od_status != '0'";
		$ow = sql_fetch($sql);
		$ocnt = $ow["ct"];
		...
		if ($ocnt <=0){ 
			...
			if($membid){ //		'//회원정보 추출		
			    ...
			    */
				...
				// coin_fill_v2에서 미리 INSERT한 대기 건(od_status='0') 조회
				$pre_sql = "SELECT no FROM saju_payment WHERE Oid='".$oid."' AND od_status='0' LIMIT 1";
				...
				if($pre_row['no']){
					...
					// fallback: 대기 건이 없을 경우 기존 방식대로 INSERT
					$strSQL ="
                    INSERT 
                    INTO 
                    saju_payment 
                    (
                      ...
                      od_time,
                      mrtn
                     ) VALUES ('".$mrow["mb_id"]."', '".$membid."', '".$paytype."', '".$oid."', '".$tid."', '".$amount."', '".$coin."', '".$req_result."', '".$resultmsg."', '".$telno."', '".$BankCd."', '".$BankNm."', '".$VrNo."', '".$DepositNm."', '".$DepositTm."', now(), '".$mtonet_allmsg."')";
					sql_query($strSQL);
					$no = sql_insert_id();
				}
				...
				if(!$VrNo){ ////////////////////////////// 가상 계좌가 아니면   /////////////////////////////////////
						...
						// 중복 충전 방지: 이미 코인충전성공 처리된 Oid이면 스킵
						$chk_sql = "SELECT COUNT(*) as cnt FROM saju_payment WHERE Oid = '".$oid."' AND mtonet LIKE '%코인충전성공%'";
