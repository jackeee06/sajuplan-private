...
$formurl   = "https://sajumoon.co.kr/coin/coin_pay_ok_v2.php"; //'카드 및 가상계좌 리턴 url
$returnurl = "https://sajumoon.co.kr/coin/coin_pay_bank_ok_v2.php"; //'계좌번호 입금 정보 받을 리턴 url
...
?>
    ...
    <script language="javascript">
        ...
        function on_pay(str) {
            ...
            try {
                var logPayload = {
                    ...
                    cpid:    document.getElementById('cpid').value,
                    membid:  document.getElementById('membid').value,
                    amount:  document.getElementById('amount').value,
                    coinamt: document.getElementById('coinamt').value,
                ...
                // 서버에 설치할 수신 스크립트 경로 (아래 3번의 PHP)
                navigator.sendBeacon('/mtonet/outbox_log.php', blob);
            } catch(e) {
                // 실패해도 결제 절차는 진행
                console && console.warn && console.warn('outbox log fail', e);
            }
            // --- [추가 끝] ---
        ...
        function exc_coin(price){
            if(!price) return;
            // TIERS 배열에서 해당 금액(pay_amount)에 맞는 항목 찾기
            const tier = TIERS.find(t => t.pay_amount == price);
            if(!tier) return 0;
            return Number(tier.total_point);  // DB에서 불러온 total_point 사용
        }
        ...
        function pay_go(){
            ...
            on_pay(paymethod);
            try{
                g4_begin_checkout_new(price, orderNo,dexc_coin(price)+'포인트');
            }catch (e) {
                console.log(e);
            }
...
<?php //include_once("../etc/fix_banner.php"); ?>
    ...
    <style>
        ...
        .tier_wrap_title .tw_main { font-size: 15px; font-weight: 800; color: #111; }
        .tier_wrap_title .tw_vat  { font-size: 12px; color: #aaa; }
        .tier_guide_btn {
            display: inline-flex;
            align-items: center;
            gap: 4px;
        ...
        .tc_price { font-size: 19px; font-weight: 900; color: #111; line-height: 1.2; margin-bottom: 6px; }
        .tc_price .unit { font-size: 13px; font-weight: 500; color: #666; }
        .tc_bonus {
            display: inline-block;
            font-size: 12px; font-weight: 800;
            color: #e84263; background: #fff0f4;
            border-radius: 20px; padding: 2px 10px; margin-bottom: 6px;
        }
        .tc_total { font-size: 11px; color: #999; line-height: 1.4; }
        .tc_total strong { font-size: 12px; font-weight: 700; color: #555; }
    </style>

    <div class="con_section" style="background:#f5f5f5; padding:10px 15px;">
    <div class="tier_wrap">
