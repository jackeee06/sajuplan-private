...
if (!$is_member) {
    alert_close("서비스문의는 회원만 작성이 가능합니다.");
}
...
else if ($w == "d")
{
    if (!$is_admin)
    {
        ...
        $row = sql_fetch($sql);
        if (!$row)
            alert("자신의 서비스문의만 삭제하실 수 있습니다.");

        if ($row['iq_answer'])
            alert("답변이 있는 서비스문의는 삭제하실 수 없습니다.");
    }
