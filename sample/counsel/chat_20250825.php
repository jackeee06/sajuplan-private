        ...
        conn.onmessage = function (evt) {
            ...
            switch (js.CmdTp) {
                ...
                case "room_in_noti": { // 방입장 통지
                    ...
                    // 표기명 결정 (회원화면=상담사명, 상담사화면=회원명)
                    const displayName = ($('#is_csr').val() == 'N') ? $('#csrname').val() : $('#mbnick').val();
                    const safeName = escapeHtml(displayName);

                    // 서버 상태를 보고 '재입장' 여부 판단
                    /*getRoomFlags(function(res){
                        const isRejoin = res && res.success && (res.rejoin === 'Y' || res.try_out === 'Y');
                        const sysMsg = `[${safeName}]님이 ${isRejoin && res.use_tiem > 0 ? '재입장' : '입장'}했습니다.`;
                    ...
                    break;*/
                    getRoomFlags(function(res){
                        const flags   = (res && res.success) ? res : {};
                        const isRejoin = (flags.rejoin === 'Y' || flags.try_out === 'Y');
                        const usedSec  = Number(flags.use_time || 0);

                        // 닉네임은 반드시 escape
                        const safeNameEsc = escapeHtml(safeName);

                        let sysMsg;
                        let isHtml = false;

                        if (usedSec > 0) {
                            // 이미 시간이 한 번이라도 차감된 방이라면: 재입장/입장
                            sysMsg = `[${safeName}]님이 ${isRejoin ? '재입장' : '입장'}했습니다.`;
                            isHtml = false;
                        } else {
                            // 아직 차감이 시작되지 않은 케이스
                            // 아직 차감 시작 전 → 안내문 (HTML 줄바꿈 사용)
                            sysMsg = `[${safeNameEsc}]님이 입장하면 포인트 차감이 시작됩니다. 조금만 기다려주세요.`;
                            isHtml = true;
                        }
                    ...
                    getRoomFlags(function (res) {
                        ...
                        if (res && res.success) {
                            if (res.status === 'DISCONNECT') {
                                tail = '상담을 종료했습니다.';
                            } else if (res.try_out === 'Y' || window.__leftByBeacon__) {
                                tail = '잠시 이탈했습니다.'; // leaveRoomSilent와 레이스일 때도 이 문구
                            } else {
                                tail = '퇴장했습니다.';
                            }
                        }
                        const sysMsg = `[${safeName}]님이 ${tail}`;   // ← 반드시 문구를 만든다
