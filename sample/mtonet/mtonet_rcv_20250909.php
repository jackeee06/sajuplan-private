...
// 콜이고 원본 금액이 1,000 이하이면 환불 대상
        $refund_eligible = $is_call && $rawAmtInt > 0 && $rawAmtInt <= 1000;
        ...
        $sql = "insert into platform_consulting(`mb_id`, `cpid`, `csrid`, `dtmfno`, `start`, `end`, `from`,
                               ...
                               `wr_datetime`, `membid`, `p_gubun`, `mrtn`, `roomid`, `skip_charge`)
                                ...
                                '" . $arr["end"] . "', '" . $arr["from"] . "' ,'" . $arr["to"] . "', '" . $arr["reason"] . "', '" . $arr["telno"] . "',
                                 ...
                                 now(), '" . $arr["membid"] . "', '" . $p_gubun . "', '" . $mtonet_allmsg . "', '" . $arr["roomid"] . "', '" . $skip_charge . "')";
        sql_query($sql);
        $no = sql_insert_id();
            ...
            if (!empty($arr_proc["membid"])) {
                ...
                if ($m_id !== '') {
                    insert_point(
                        ...
                        '@platform_consulting',
                        $m_id,
                        $no . '@상담코인 차감 :' . date("Y-m-d H:i:s") . "@" . $p_gubun
                    );
                }
            }
            // 상담사 증가
            if (!empty($csrid) && !empty($minfo["mb_id"])) {
                insert_point(
                    $minfo["mb_id"],
                    $amtInt,
                    $svc_type . '상담코인 증가',
                    '@platform_consulting',
                    $minfo["mb_id"],
                    $no . '@상담코인 증가 :' . date("Y-m-d H:i:s") . "@" . $p_gubun
                );
            }
        }
        /*
