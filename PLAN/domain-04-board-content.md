...
사주문1의 라이브 관리자(`sample/adm`)는 그누보드5(영카트5) 표준 구조를 그대로 사용한다. 게시판 관리는 **모든 게시판의 메타정보를 단일 테이블 `g5_board`(95개 컬럼)에 저장**, 글은 **게시판 슬러그별 별도 물리 테이블 `g5_write_<bo_table>`을 동적으로 CREATE/DROP**하는 모델이다.
...
6. **이슈**:
   ...
   - **78KB 모놀리식 폼** — 95컬럼 영카트 옵션 대부분(skin, include_head, sideview, RSS, 공지, captcha 등) 사주문에선 미사용.
   - 본인확인(`bo_use_cert`), SNS 공유(`bo_use_sns`), 캡차 (`bo_use_captcha`) 등 사주문 운영에서 안 쓰는 옵션 다수.
7. **운영 사용 추정**: 부분 사용. 게시판 추가/제목 수정/포인트 수정 등 상위 5~10개 옵션만 실사용 추정. 디자인 옵션(스킨/갤러리/list_view/include_head)은 사주문 SSR 페이지엔 무관.
8. **이관**:
...
3. **이슈**:
   ...
   - 디렉토리 복사가 파일 시스템 의존.
4. **운영**: **거의 미사용 추정**. 사주문 운영팀이 게시판을 복제하는 시나리오는 사실상 없음(슬러그 18개 고정).
...
5. **운영 사용 추정**: **사용 중 추정** — 운세/후기 콘텐츠를 엑셀로 일괄 업로드하는 작업은 사주문에서 자주 일어나는 작업으로 보임 (sample 파일 2개를 둔 것으로 추정 가능).
...
- **운영**: **거의 미사용 추정** — 사주문은 회원 차별 접근 게시판이 없을 가능성 큼. `gr_use_access=1` 그룹이 운영에 있는지 확인 필요.
...
**투표 운영 사용 추정**: **거의 미사용** — 사주문은 콘텐츠 사이트로 투표 기능이 핵심 UX가 아님. 라이브에 폼은 살아있어도 실제 운영 활용은 제로에 가까움. 그누보드 기본 기능 잔존물.
...
- **운영**: **미사용**. 사주문은 PG가 이미 연동돼 있어 신규 신청 안내 페이지가 의미 없음.
- **이관**: **컷**.

#### F-2. `service_list.php` (285줄)

- **역할**: 코드 헤더는 `정기세차신청` — 자동차 정기 세차 서비스 관리 화면. **사주문과 무관한 다른 사이트의 잔존 코드**.
...
- **운영**: 외부 플러그인 연동용. 현재 사주문에서 등록된 plugin이 있는지 확인 필요. 일반적으로 미사용.
...
12. **service.php** — 그누보드/영카트 기본 PG/SMS 가입 안내 광고 페이지. 사주문 운영 무관.

### 그누보드 표준이지만 사주문에선 안 쓰는 기능
13. **board_copy** — 게시판이 18개로 고정되어 복제 시나리오 거의 없음.
14. **board_excel** — 단, fortune/review만은 실사용 가능성. 그 외 슬러그용 엑셀 업로드는 의미 적음.
15. **poll_*** — 투표 기능. 사주문 운영에서 활용 사례 없음.
16. **boardgroupmember_*** — 그룹별 회원 접근권한. 사주문 정책상 거의 무용.
