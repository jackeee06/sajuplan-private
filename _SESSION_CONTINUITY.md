...
**사주플랜 (sajuplan.com) 정밀 audit + 보안 강화 + 운영 매뉴얼 + E2E 자동 테스트 인프라까지 완성**. Critical 12건 + 보안 15건 + 18개 DB invariant + 50개 자동 클릭 테스트 모두 100% 통과.
...
| prod 서버 | `104.64.128.103` (`sajuplan.com`, `api.sajuplan.com`) |
...
1. **BizM 콘솔에 `ops_admin_alert` 템플릿 등록** — 사장님이 개발자에게 내일 요청
   - 본문 (DB `alimtalk_template` 와 100% 동일해야): `[사주플랜 운영 알림]\n\n유형: #{category}\n시각: #{at}\n\n#{detail}`
...
# OpsAlert 테스트
curl -H 'X-Cron-Token: $TOKEN' https://api.sajuplan.com/api/cron/test-alert
```
