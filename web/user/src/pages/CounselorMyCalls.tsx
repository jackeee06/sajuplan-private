...
import { MOCK_COUNSELOR_CALLS, type ConsultLog } from '../data/counselorMyPage'
 ...
 *
 * 카드: 이름·일자·길이 + 메모/휴지통 / 시작·완료시간 / 후기작성 상태
 *  - 후기작성 완료: "후기 답변 작성하기" 보라 풀 버튼
 */
export default function CounselorMyCalls() {
  const navigate = useNavigate()
  const [page, setPage] = useState(1)
  const items = MOCK_COUNSELOR_CALLS
        ...
        <ul className="flex flex-col">
          {pageItems.map((it) => (
            <li key={it.id} className="border-b border-[#F3F4F6]">
              <CallCard log={it} onMemo={() => navigate(`/counselor/mypage/calls/${it.id}/memo`)} />
            </li>
          ))}
        </ul>
...
function CallCard({ log, onMemo }: { log: ConsultLog; onMemo: () => void }) {
