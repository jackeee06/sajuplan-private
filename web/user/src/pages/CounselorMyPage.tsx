  ...
  return (
    <div className="mobile-frame flex flex-col pb-[100px]">
      ...
      <main className="flex-1 px-4 space-y-3">
        ...
        {/* ③ 🔔 답변 대기 — 문의/후기 각각 한 줄(줄 전체 클릭) + 최근 항목 미리보기 → 정확한 화면으로 이동.
            (이전: 합산 카드 하나가 후기까지 문의 화면으로 보내 "눌러도 안 뜸" 혼란 → 2줄 분리로 해소) */}
        {(() => {
          const qna = pendingCounts?.pending_qna ?? 0
          const review = pendingCounts?.pending_review ?? 0
          const total = qna + review
          const hasPending = total > 0
          const Chevron = () => (
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" className="shrink-0 text-rose-300" aria-hidden>
              <path d="M9 6l6 6-6 6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          )
          return (
            <section className={`rounded-[16px] border p-4 ${hasPending ? 'border-rose-300 bg-rose-50/60' : 'border-amber-200 bg-amber-50/40'}`}>
              <div className="flex items-center justify-between">
                ...
                {hasPending && <span className="text-[11px] font-medium text-rose-600">총 {total}건</span>}
              </div>
              ...
              {hasPending ? (
                <div className="mt-3 flex flex-col gap-2">
                  {qna > 0 && (
                    <Link
                      ...
                      data-testid="pending-qna-row"
                      className="flex items-center justify-between gap-2 rounded-xl bg-white border border-rose-100 px-3 py-2.5 hover:bg-rose-50/60 active:bg-rose-100/60 transition"
                    >
                      <div className="min-w-0">
                        <div className="text-[13px] font-semibold text-rose-700">💬 문의 {qna}건</div>
                        {pendingCounts?.recent_qna_title && (
                          <div className="mt-0.5 text-[12px] text-[#6A7282] truncate">최근: 『{pendingCounts.recent_qna_title}』</div>
                        )}
                      </div>
                      <Chevron />
                    </Link>
                  ...
                  {review > 0 && (
                    <Link
                      to="/counselor/mypage/reviews"
                      data-testid="pending-review-row"
                      className="flex items-center justify-between gap-2 rounded-xl bg-white border border-rose-100 px-3 py-2.5 hover:bg-rose-50/60 active:bg-rose-100/60 transition"
                    >
                      <div className="min-w-0">
                        <div className="text-[13px] font-semibold text-rose-700">⭐ 후기 {review}건</div>
                        {pendingCounts?.recent_review_title && (
                          <div className="mt-0.5 text-[12px] text-[#6A7282] truncate">최근: 『{pendingCounts.recent_review_title}』</div>
                        )}
                      </div>
                      <Chevron />
                    </Link>
                ...
                <p className="mt-1 text-[12px] text-amber-700">새로 답변할 문의·후기가 없습니다.</p>
              )}
            </section>
          )
        })()}
