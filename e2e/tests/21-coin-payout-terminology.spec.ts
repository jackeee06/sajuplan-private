...
test.describe(`회원/상담사 용어 분리 정책 회귀 방지 (${TARGET})`, () => {
  ...
  test('5) 세후 실수령 표시 존재 (수익금 페이지 핵심 정보)', async ({ page }) => {
    // [2026-06-14] 헤더 카드를 3숫자(내 수익금/선지급 가능/이번 달 수익)로 개편하면서
    //   세후 실수령 라벨이 헤더 "실수령 예상" → 실시간 정산 탭 "예상 실수령액" 으로 이동.
    //   보호 의도(상담사가 세후 받을 금액을 본다)는 유지 — 둘 중 하나라도 있으면 통과.
    const src = await getAllScripts(page)
    expect(src, '세후 실수령 표시 사라짐 — 상담사 신뢰 보호 회귀').toMatch(/예상 실수령액|실수령 예상/)
  })
