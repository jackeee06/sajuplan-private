...
## ⚠️ 2026-06-11 정정 — 안드·iOS 둘 다 정상 (이 백로그 폐기)

앱 이동은 **푸시/FCM 이동 방식**으로 **안드로이드·iOS 둘 다 동일 기능으로 등록·구현 완료**(사장님 확인). 카카오 알림톡 클릭 → 앱이 꺼져 있어도 켜지고 **정확한 페이지로 이동**됨.
→ 아래의 "AndroidManifest intent filter / assetlinks.json / apple-app-site-association / RN Linking 처리" 계획은 **진행 불필요(폐기).** 역사적 참고용.

- **BizM 설정·백엔드·서버·사주플랜 코드 = 변경 0** (정상). 안드가 정상이므로 BizM/서버 문제가 아님.
- 일부 아이폰 에러 제보(2026-06-11)는 iOS 기능 자체는 등록돼 있으므로, **그 단말의 앱 미설치/구버전 등 환경 가능성** — 그 아이폰의 앱 설치·버전부터 확인. [[mobile-deep-link-status]]
- ⚠️ repo `mobile/` grep 으로 "미구현" 단정 금지 — 배포 APK/IPA 가 정답.
