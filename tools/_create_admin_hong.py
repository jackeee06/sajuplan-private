"""슈퍼관리자 계정 생성: hong / ara!! (member 테이블, role=admin, level=10, is_super=TRUE)
permissions.service.ts createAdmin 와 동일 INSERT + is_super=true.
...
try:
    ...
    # 3) INSERT (role=admin, level=10, is_super=TRUE — 슈퍼관리자)
    ins = (
        f"psql \"{DB_URL}\" -c \"INSERT INTO member "
        f"(mb_id, password, name, nickname, role, level, is_super) "
        f"VALUES ('{MB_ID}', '{pw_hash}', '{NAME}', '{NICK}', 'admin', 10, true)\""
    )
    o, e, rc = ssh_run(client, ins)
    print("--- INSERT ---\n", o.strip() or e.strip(), "rc:", rc)

    # 4) bcrypt 검증 (방금 만든 해시로 ara!! 가 매칭되는지)
    verify_cmd = (
        f"cd {API_DIR} && node -e \""
        f"const b=require('bcrypt');"
        f"b.compare('{PLAIN_PW}', '{pw_hash}').then(r=>console.log('compare:', r));\""
    )
    o, e, _ = ssh_run(client, verify_cmd)
    print("--- bcrypt compare 검증 ---\n", o.strip(), e.strip())
