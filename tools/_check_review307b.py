...
try:
    q(c,"SELECT table_name FROM information_schema.tables WHERE table_name ILIKE '%review%' OR table_name ILIKE '%post%' ORDER BY 1","후기 관련 테이블명")
    q(c,"SELECT column_name FROM information_schema.columns WHERE table_name='alimtalk_log' ORDER BY 1","alimtalk_log 컬럼")
finally:
    c.close()
print("done")
