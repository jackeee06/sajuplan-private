...
try:
    q(c,"SELECT column_name FROM information_schema.columns WHERE table_name='post_review' ORDER BY ordinal_position","post_review 컬럼")
    q(c,"SELECT id, counselor_id, member_id, is_best, deleted_at FROM post_review WHERE id=97","post_review id=97")
finally:
    c.close()
print("\n완료")
