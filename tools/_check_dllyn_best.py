...
try:
    q(c,"SELECT member_id, content, earn_point, rel_action, created_at FROM point_history WHERE rel_action='review_best:223'","review_best:223 지급 내역 (있으면 테스트가 실지급)")
    q(c,"SELECT id, member_id, mb_id, is_admin_best FROM post_review WHERE id=223","후기 223 현재 admin_best 상태")
    q(c,"SELECT member_id, free_balance, paid_balance, total_earned FROM point WHERE member_id=176","dllyn(176) 잔액")
finally:
    c.close()
print("\n완료")
