"""Phase 14 — page 테이블에 terms / privacy 페이지 시드.

기존 page 테이블 + /api/user/pages/:slug + 어드민 contents 메뉴 재사용.
"""
...
본 약관은 사주문(이하 "회사")이 제공하는 사주/타로/신점 상담 서비스(이하 "서비스")의 이용과 관련하여 회사와 회원의 권리·의무 및 책임사항을 규정함을 목적으로 합니다.
...
(※ 본 본문은 초안입니다. 정식 약관은 법무 검토 후 어드민 → 콘텐츠 관리에서 교체하세요.)"""
...
(※ 본 본문은 초안입니다. 정식 처리방침은 법무 검토 후 어드민 → 콘텐츠 관리에서 교체하세요.)"""
...
def make_sql():
    ...
    return f"""
INSERT INTO page (slug, title, content, mobile_content, use_html, is_active) VALUES
  ('terms',   '회원가입약관',     '{terms_q}',   '{terms_q}',   false, true),
  ('privacy', '개인정보처리방침', '{privacy_q}', '{privacy_q}', false, true)
ON CONFLICT (slug) DO NOTHING;

SELECT '=== legal pages ===' AS section;
SELECT slug, title, is_active, LENGTH(content) AS content_len, updated_at::text
  FROM page WHERE slug IN ('terms', 'privacy') ORDER BY slug;
"""
