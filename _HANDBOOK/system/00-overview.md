...
**월 정산** (🔴 전환 중 — 자동 cron 꺼둠, 개별 [정산하기] 버튼)
미정산 수익금(earning_balance, 등급률·추천수익 이미 반영됨) 합산 → 선지급 차감 → **원천세 3.3%만 차감** → 실지급액 → 운영자 수작업 계좌이체.
(부가세 10%·회선비는 옛 php 잔재라 제거됨 — 떼는 건 원천세 3.3% 하나뿐.)
